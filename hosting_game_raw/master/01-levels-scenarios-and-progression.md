...
- **Three-tier escalation / Three-era campaign / Zoom-as-scale (the compact spine).** A shorter
  restatement of the same ladder as three acts: **Act 1 Garage** (one machine, one rack), **Act 2
  Operator** (a colo cage, real topology), **Act 3 Hypercloud** (multi-region). Each act replays the
  same hosting-type beats at bigger scale so the player masters one *layer* before the next, and
  difficulty comes from new systems rather than stat inflation. At every act boundary the camera
  zooms out one level (chip → rack → floor → building → city → world map) and a whole new resource
  class unlocks (power → bandwidth → sovereignty).
- **One new physical truth in, one convenience out.** Each tier *introduces one new physical truth*
  (power, cooling, cabling, fiber, water, security) and *removes one convenience* (reboots get
  slower, blast radius gets bigger). Scale mechanically changes three things: **(a) abstraction** —
  at single-server you click processes, at rack you click servers, at DC you click tiers, at
  multi-DC you click *policies*; **(b) parallelism** — the number of simultaneous incidents;
  **(c) your hands** — direct builds vs staff and auto-scalers. Early = being the sysadmin at 3am;
  late = commanding from the NOC wall. Unlock pacing follows: **new buildables only appear at the
  scale that can host them.**
- **Bridge the two ladders explicitly (one table, not two poems).** Act 1 = tiers 0–2 (*hands*),
  Act 2 = tiers 3–4 (*teams*), Act 3 = tiers 5–6 (*policies*); and assign one new physical truth per
  tier — t1 power, t2 cabling, t3 cooling, t4 bandwidth, t5 sovereignty, t6 law.
- **The sysadmin ladder, stated as a design rule.** Tier 0 single box → 1 one rack → 2 half cage +
  LB/DB split → 3 full cage, ASN-adjacent, scrubbing → 4 multiple halls → 5 multiple buildings →
  6 multiple regions.
- **The level-named ladder.** The same rungs with level names attached: *First Box* (closet shared
  server) → *Closet Operator* (2–3 rackmounts colocated, cross-server DB-behind-web, the landlord's
  SLA meter) → *Rack Our Own* (full cage in a carrier hotel: UPS/PDU/cage-lock buildables, the first
  carriers-vs-transit uplink choice) → *Region Ruler* (hot-standby redundancy, tiered-SLA enterprise
  clients) → *Anywhere, Everywhere* (anycast world map plus per-site interiors).
- **The ladder as a mechanic, not set dressing.** Each rung multiplies *both* flows (more customers
  to catch, more surface to defend) and unlocks placement rules (S0–S1 one floorplan; S4+ racks
  inside rooms inside sites). **The ladder is the difficulty curve's X-axis; hosting type is the
  Y-axis.**
- **Hard-gated five-rung variant.** L1 Closet → L2 Rack → L3 Half Datacenter → L4 Full Datacenter →
  L5 Multi-Region, where each tier adds grid, simultaneous ingress lanes and power/cooling caps, and
  **the tier gates which buildables exist at all** (no second upstream carrier on L1).
- **One-axis-per-rung pacing.** closet → colo rack → half a hall → own DC hall → two DCs → regional
  mesh → global anycast, each rung introducing exactly ONE new axis (power → cooling → network →
  compliance → orchestration → geopolitics).
- **The system ladders that run alongside the scale ladder.** Networking (managed switches → BGP →
  anycast) · redundancy (1 PSU → 2 PSU → N+1 → 2N) · automation (manual → scripts → config
  management → IaC → self-healing) · observability (ping → logs → metrics → traces → AIOps). A rung
  on the scale ladder is also a rung on each of these.
- **Two extra rungs worth naming.** *Basement Rack Era* — 2–3 self-owned rackmounts in your own
  basement on a 100Mbit uplink, shared hosting plus the first mail server; introduces **upkeep**
  (electricity meter, heat) and the first noisy-neighbor event. *Single Datacenter Operator* — your
  own DC with racks, hallways, staff desks, SOC, NOC, generator and a biometric door unlock, where
  the economy flips from survival to margin management.
- **Act structures (three candidate shapes, all compatible with the spine).**
  - **Four acts**: I one server, II colo rack(s), III own datacenter, IV multi-region — each
    introducing ONE new hosting type as flavour and one new systemic idea (upkeep, facilities/power,
    cross-region state), with act finales as boss incidents combining everything taught.
  - **Three acts × ~8 levels**: scrappy survival / growth economics / big-operator risk portfolio,
    separated by a "founder screen" recapping reputation and net worth carried forward.
  - **A 20-mission "Garage to Global" campaign** whose mid-campaign act *pivots the company* (the
    shared-hosting act ends with your IP blacklisted, forcing the VPS act), with a final level of
    four regions run by an autonomous fleet.
...
- **"Noisy-Neighbor Graduation" — the shared→VPS tutorial framing.** Five customers on one box; one
  installs a Bitcoin miner and everyone's sites crawl. The level teaches **cgroup isolation as the
  "tower" you place** — the first time a defense is a *partition* rather than a wall.
- **The neighbor is IO, not CPU (correction).** cgroups fix CPU, but the *real* noisy neighbour on
  every shared host is **disk I/O** (one tenant's backup scan murders everyone's latency) and the
  **connection table**. Add an IO-throttle (ionice/BFQ-flavoured) as the actual tower; the lesson
  then carries forward intact to the VPS and cloud tiers instead of being re-taught.
...
- **"Garage Colocation" / "First Rack" / "The Landlord's Basement" — the small-operator opener.**
  You rent 2 cabinets, or a 42U cage, in someone else's datacenter. Facility power and cooling are
  *given*; a noisy-neighbor mechanic (tenants' bursts steal bandwidth) teaches QoS; per-kW billing,
  the uplink cross-connect and remote-hands tickets introduce **"you're not in the room when it
  breaks"** — hardware events cost hours unless you paid for smart hands.
  - *Basement variant:* a damp room, the first cable-management moment (tidy a dangling cable for
    free style points), and an ambient drip that occasionally shorts an unshielded cable — a
    diegetic tutorial for environment threats.
  - *6U variant:* shared power and cooling you don't control, you own only what's bolted in your
    rails; introduces remote-hands cost, IPMI, and the fact that **you can't physically see your
    gear, so outages are *inferred*.**
  - *Two-42U-racks variant:* new verbs — order cross-connects, pay remote-hands fees (per visit;
    every truck roll stings), schedule your own hardware deliveries — and new threats you *can't*
    fix remotely: bad RAM reseats, snapped bezels, a tech bumping a power strip. **Redundancy exists
    because the building is not yours.**
  - *"Half a Rack" variant:* physical-layer threats (power dips, a failed PSU you must drive across
    town to replace) and the metered-power upkeep economy.
  - *"Landlord's Closet" variant:* one beefy machine with dozens of tiny sites packed on it —
    noisy-neighbor events and XSS worms hopping between tenants; the first real churn pressure.
  - *"First Colocation Cage" variant:* you build cages and cross-connects instead of facilities;
    power and cooling are *rented* — a fixed monthly drain, but a carrier-departure event can cut
    your uplink. The concept being taught is **"things you don't control."**
- **Power, not RU, is the garage's god.** The *real* wall is 20A @ 208V ≈ 3.8kW continuous per
  circuit — a single 2U GPU box is half your feed. Make the hard constraint a literal breaker that
  trips (an audible *clunk*, one rack goes dark) when the player stacks dense gear: **RU clutter as
  the joke, kW as the lesson.**
- **The landlord is on screen.** In colocation levels the *datacenter owner* appears as a silhouette
  behind glass at the top of the screen, arms crossed. Facility rules you break (a power overage)
  make the silhouette tap the glass.
...
- **"First Real Lease" — the founding beat.** Half a row in a real DC introduces the **load balancer
  + DB split**, and states the game's thesis without text: *the instant the DB is placed, red
  port-LEDs visibly bloom on the chassis and SQL-injection threats start appearing at the gate.*
  The capability/surface tradeoff, taught in one animation.
- **"Channel Lock-In" — vertical progression inside one building.** half rack → full rack → cage →
  hall → multi-hall, where each step unlocks bigger tenants but forces bigger commitments: **the
  landlord's contract raises your fixed cost whether or not you fill the cages.** Space speculation
  is a real colo gamble, and this is where the player first makes it.
...
- **"Dual-Region Failover" / "Regional Empire" — the campaign-finale framing.** Two full maps on one
  screen linked by a dark-fiber cable; enemies hit both; either region can die but the company
  survives if the other absorbs load. **Failover is a player-triggered mega-ability** with a
  cooldown, a cost, and a data-consistency penalty (lag = a possible split-brain event to resolve).
  Above a certain scale the map becomes a **globe**: DCs are nodes, subsea cables are the paths,
  threats arrive as routing attacks and regional outages, and customers are *regions* served from
  the nearest healthy node — the same dual flow zoomed out, with **latency = distance, literally.**
- **The "Constellation" awe beat.** The camera leaves the building for the first time; DCs glow on a
  dark world map joined by undersea arcs; cross-region threats travel the arcs like torpedoes.
- **"Regional Rush" (the opening beat of the tier).** Unlock a second region and customers arrive
  wanting "latency to São Paulo" while existing tenants get a "closer region available" upsell
  nudge; failure modes are split-brain during cross-region replication and **duplicated-invoice
  disasters**.
- **"Regional Expansion" (two economies).** Different power costs, latency maps, attacker pools and
  customer bases per region; inter-region replication becomes a buildable and **data sovereignty
  becomes a threat-gated requirement** ("EU customers need EU data — build it or lose them").
- **"Two-Region Arbitrage" (late-game optimization).** Same product, two regions — power costs 3×
  different, wage pools differ, FX swings, one has an IX peering exchange and the other strict data
  residency; route new customers to whichever region's unit economics win *this month*. Rewards
  mastery of the COGS map rather than of combat.
- **"The Region" (traffic engineering as the meta-layer).** Multiple DC maps on one screen (or linked
  via a WAN map): which site serves whom, where the DR copy lives, who gets the GPUs. **Latency
  rings between sites drive customer eligibility — GDPR as geography.**
- **"Regional Outage Chain."** One region goes dark *in fog of war* — you get alerts, not sight —
  and traffic must fail over to the remaining regions without melting them: they're sized for N+1 of
  one, not for one on vacation.
- **"Continental."** Region-specific threats: one jurisdiction's regulator, one ocean's cable cut,
  one continent's botnet herd, with the full tech tree in play.
- **Two-board finale.** Regions A and B split the screen with a **finite-capacity WAN link** between
  them — customers route to either, but the link is the chokepoint (latency = churn on cross-routed
  customers); the anycast unlock makes routing automatic but expensive.
- **Visual — the Dark Forest map.** A night-globe earth with undersea arcs as glowing strings between
  datacenter lights, threats arriving as shadow-ships on the ocean paths, and latency drawn as
  customer sprites *trudging the arc length*. Between acts, a region-unlock map lights up POPs as
  you earn them; each dot pulses in its hosting type's signature colour and arcs animate between
  them as backbone builds complete.
...
- **Each scale tier is a different *room*, not a different zoom.** One beige tower on a carpeted desk
  → a closet with a 6U rack and a sticker on the door → a row of racks with zip-tied cabling → a
  full DC hall with raised floor → a multi-region map — **each with its own floor material, ceiling
  height, ambient light and camera altitude.**
- **The Tier-Up Pull-Back (the signature motion moment).** When the player crosses a scale threshold
  the camera does *one continuous dramatic pull-back* revealing the bigger room, the HUD chrome swaps
  to the next tier's style, and the ambient sound bed crossfades. At level completion the camera
  keeps pulling back to reveal the *next* level's scope (one desk → server room → warehouse → campus
  → earth), as if the whole game were one zoom sequence with no loading screens.
- **Every level opens with a scale-appropriate establishing zoom-in** (macro on LEDs and connectors →
  room → floor plan → building cutaway → dark-mode globe with light arcs), and HUD font and line
  weight scale a half-step between altitudes.
- **Level-start zoom *is* the difficulty tell.** Early levels open zoomed in on one rack; late levels
  open at parking-lot zoom where racks are grains of rice. A **map-scale badge** (1U → 40U → 1 rack →
  1 room → 1 building → campus → region) lives in the HUD corner and grows, and a scoring bonus
  rewards defending at wide zoom ("you kept the WHOLE campus green").
- **Persistent-world continuity.** A completed small level physically becomes *one rack* in the next
  level's room — saved structures seed the next scene, teaching the scale ladder with no tutorial
  text.
- **"The Hall" mid-campaign beat.** At 40+ racks under one roof, **screen space itself becomes the
  resource** — this is where overlay tooling is introduced, because the map is now unreadable
  without it.
- **The act-boundary employee photo wall.** As the camera rises at an act boundary, one staff avatar
  waves from below and the view passes an in-world photo wall where the act's worst incident is
  already framed — progression, memory and scale in one move.
- **Scale-shift *inside* a level.** One mission starts at tier 1 (a rack) and ends at tier 4 (a
  datacenter): the camera and the UI abstraction physically change mid-level as you win (placement
  grid → adjacency graph → topology graph). Progression *felt* as zooming out; a campaign
  centerpiece.
...
- **The business-lifecycle spine (an alternative framing of the same rungs).** Levels follow the
  stages every host actually passes through: **hobby → reseller → real company → multi-product brand
  → regional player → multi-DC enterprise → exit.**
- **"Company Lifecycle" acts.** Act I closet → shared host; Act II colo cage → VPS provider; Act III
  many racks → CDN/streaming/GPU branch choices; Act IV multi-region operator. Final level: **you
  are the vendor now** — a hosting outage somewhere makes *you* the target of an angry wave while
  you run 12 racks, and there is nobody left to complain to.
- **Revenue-model unlocks — progression gates billing, not just tech.** Scale progression should gate
  **billing models**: manual invoicing → flat prepaid → metered → committed/contracted → marketplace
  co-sell. Each unlock reshapes the money HUD, so the player graduates from *collecting* to
  *managing float, commitments and receivables* — progression that changes the verbs, not the
  numbers.
...
### "First Byte" / "The Closet" / "The First Modem" — the unified opener
The campaign's actual first playable level, sitting just below Tier 0/Tier 1 on the ladder: one
aging tower server in a coat closet, its green power LED reading as a tiny town centre.

**How it works:** Five friend-and-family WordPress sites (or one fan-site customer) live on it. The
customer path is **a phone cord snaking under the door.** Teach the bare loop on one screen with
about six entities: place the firewall/password-gate on the single uplink, serve the trickle, answer
one support ticket, survive three script-kiddie probes or a lone port scanner crawling the cord.
**Variant framing:** 1994 dial-up era, zero economy pressure — *the level cannot be lost, only
rated.*
**Give it numbers:** an explicit tutorial exemption — the mom-unplug event and bankruptcy are both
disabled, and the rating runs on three visible thresholds (serve ≥80% of arrivals, end with ≥$40
reserve, zero permanent bounces = 3★). Fix the internal clash "5 friend sites" vs "one fan-site
customer" by picking **the five**, each teaching one bounce cause: slow, dead, hacked, spammy,
happy.
**Interacts with:** Tier 0 and Tier 1 above (this is the rung beneath them), §1.11 `The Cold Open
Level`, §1.9 `The Cold Start Minute`.

**Variations and additions**

- **"One Box in a Closet."** The scale floor of the whole game — one secondhand tower under a desk,
  three friends' websites, threats of one bored script kiddie and one failing fan; teaches
  **place → connect → patch → profit.**
- **One tower, one 100Mbit uplink, three customers** (a baker, a band, a cousin's fan site); teaches
  power, uplink, cert expiry and tickets; threats are one script kiddy plus one dead drive.
- **Single dedicated server, one uplink, yourself.** Place a web server, accept first visitors,
  `ps aux | grep` a cryptominer by hand, reboot once — **no automation exists yet**; threats are
  port scanners (harmless tutorial scouts) plus one brute-forcer.
- **Vocabulary-first framing.** Two "lanes" (HTTP in, slow outbound), one web server, one dial-up-ish
  uplink; teaches placement, cabling and the customer pipeline with near-zero friction.
- **The diorama framing.** One literal tower case rendered enormous and cut open like a diorama: you
  defend a motherboard, threats are dust bunnies and a surge from the wall socket, and customers are
  tiny figures climbing a ladder to the USB port. Establishes the **"entities ride cables"** motif
  that the whole game then reuses.
- **Unlosable-ish framing.** Failure simply retries the wave; the level teaches the core verb pair —
  place a defense, keep the customer path fast.
- **Teach the map metaphor before combat.** A "free intro to sysadmin course" framed as a
  text-adventure shell — the player literally types `ping` and sees the customer dot move.
- **The scripted teaching failure.** In level 3, a guaranteed unavoidable failure lands mid-wave (the
  disk fills): the camera takes you to the console log, shows exactly why, then **refunds a nine** —
  a failure that teaches instead of punishing, and it belongs early, because vocabulary learned
  through loss sticks.
- **Era-flashback framing.** 1994, a garage, one 486, a hand-lettered price list, tutorial text on
  legal-pad sticky notes — it doubles as tone-setting: *this game loves its history.*
- **The cord as meter (visual).** The phone cord **tautens and hums** with bandwidth (14.4k = a lazy
  sag, 56k = a singing straight line); incoming calls ring a wall-mounted handset with a caller-ID
  chit; the firewall tower clips on inline as an alligator-clip unit. **The entire tutorial reads
  from one cable.**
- **"Day One at the New Job" framing.** The tutorial as a wave-0 level with a mentor/narrator who
  explains the mechanics diegetically on a tiny inherited site, then hands off control.

#### Mom-unplug loss event — *CONFLICTING*
Whether the closet tutorial's signature joke hazard is allowed to be random. The dispute is about
what a random, unforeshadowed loss in the first twenty minutes teaches a new player.

**Position A — the random loss event** *(opencode, wave-1 sysadmin)*: the tutorial carries a random
"mom unplugs it" loss event; the joke lands because it is out of your control, which is also the
lesson about hosting on hardware you do not own.

**Position B — deterministic and foreshadowed** *(opencode, wave-2 game designer)*: random
unloseable-feeling losses in the first 20 minutes **poison the whole game's trust.** Make the closet
hazard deterministic and telegraphed — a cat by the cord, a laundry pile near the plug, visible two
beats before it matters. The joke lands *because* players are learning the telegraph language that
the rest of the game runs on.

#### Dial-up bottleneck model — *CONFLICTING*
What the scarce resource actually is in the dial-up-era opener, and therefore what the player places
and upgrades.

**Position A — one uplink, one gate** *(opencode, wave-1 general)*: place the single
firewall/password-gate on the single uplink and teach the trickle on one path. Simplest possible
board; one cable, one defense, one flow.

**Position B — the lines are the bottleneck, not the modem** *(opencode, wave-2 sysadmin)*: the
scarce resource in 1994 was **POTS pairs** — one phone line per caller, each caller occupying one
for their whole session (12 modems on a T1 is closet phase two). Capacity = number of physical
lines. The single-uplink version undersells why dial-up ISPs actually failed, and the queueing
lesson it replaces is the one the rest of the game builds on.

### "Founding Day" — level 0 (company creation as a playable level)
Before the first rung of the ladder, the company itself is built in a playable scene.

**How it works:** Pick a **legal structure** (sole proprietorship / LLC / co-op), a **jurisdiction**
(a combined tax and seizure-law dial), a **hosting-type commitment**, and the founding trio of
staff. The choices set the starting economy style, **which threat families can ever appear** (a
co-op can't be VC-invested, but law enforcers like you less), and the campaign branch you start on.
Your founding type is the "from" in the first pivot.
**Interacts with:** §1.6 (Difficulty as Business Model — the faction selector this is the front-end
for), §1.12 (Franchise Map), §1.4 (`Pivot`).

### "First Invoice" / "First $10K MRR" — the business-ladder openers
The same opening rung, denominated in money instead of hardware. These are the levels the business
ladder starts on while the scale ladder is still in the closet.

**How it works — "First Invoice" (tutorial, single VPS):** One rented server, five hobby customers on
a payment-terminated plan. Teach: place a web server, accept a customer, **collect the first MRR
tick.** Threat: a single lazy port-scanner. Loss condition: a server crash with no backup → *"your
customers' sites are gone, refund storm."* **Business lesson: revenue is tiny, support is
everything.**
**How it works — "First $10K MRR" (the goal-level version):** You don't win by killing waves; **you
win by reaching $10,000 monthly recurring revenue** — a stricter variant demands it within 24
in-game months with churn under 8%/mo — before runway runs out. Threats stay gentle (script kiddies,
one noisy neighbour) precisely so the pressure lands on **pricing, funnel and churn.** It forces plan
design and retention rather than defense, which is the whole point of having a business ladder at
all.
**Interacts with:** "The parallel business ladder (CEO framing)" above (this is its L1), §1.6
Achievement gates (and the warning there about MRR-threshold gates that reward waiting), §1.1
`"First Byte" / "The Closet"` (the same moment on the scale ladder).
...
- **"Pagers Gone Wrong" — the pure ops level.** Not a build level at all: staff is finite, alerts
  fire across the map, and the mechanics are **attention budget, latency budget, triage priority and
  burnout.** The best vehicle in the game for teaching a failure taxonomy.
- **"The On-Call Rotation Level."** The player directly controls response only during their own
  *shift*; a weaker AI-controlled responder handles the off-shift hours — and **its quality visibly
  tracks how well the player wrote things down.** The level is a referendum on your runbooks.
- **"Night Watch" (wave-less variant).** Threats and customers dribble continuously and a
  paging/alert system wakes you across a sleep cycle: each alert answered well is a bonus, each
  missed one a consequence. **Attention IS the resource.**
- **"Day in the life" micro-levels.** Five-minute interstitials in first-person NOC view — triage a
  ticket queue, decide which alerts deserve waking the on-call engineer; they feed the staffing and
  morale systems of the main levels.
...
#### Inspector path: fixed or randomized — *CONFLICTING*
Whether the auditor's walk through the facility is a single authored route the player can study, or
one of several routes chosen at run time. The decision turns on whether the level is a build puzzle
solved in advance or a live triage encounter.

**Position A — a single fixed pathline** *(master, and opencode wave-1)*: the regulator walks one
fixed path through your facility like a Pac-Man ghost, drawn on the floor ahead of him as a dashed
route with timestamps so the player can plan; items he has passed keep a green chalk tick. The level
is a **sightline puzzle**: you build it away before he arrives, and you cannot hide noncompliance.

**Position B — two or three telegraphed routes, chosen live** *(opencode, wave-2 game designer)*: a
fixed route for eight minutes is passive — *a quiz, not a level.* Randomize the walk among 2–3
telegraphed routes so **live misdirection** becomes playable (schedule a "maintenance" to close a
room for one checkpoint, with an ethics/heat cost if you're caught), and make hot-fixing during the
walk a full timed triage minigame with explicit "you can't fix everything" scarcity.

**Variations and additions**

- **Type II reality check — you cannot hotfix the past.** The auditor asks for *evidence windows*
  ("show me 90 days of access reviews"). Record compliance-relevant behaviour passively during the
  build phase and have the auditor replay **your own actions** at inspection time; if the
  restore-test tower never ran, the log literally does not exist. **You can't be caught lying, only
  *unproven*.**
- **Canonical-entity note.** Unify this scenario and the §2 "Regulator Walkby" threat under **one
  Inspector entity** with a per-type gaze/behaviour table (privacy, security, financial, safety):
  scenario levels are full episodes starring it, threats are cameos. Two subtly different
  implementations of the same fantasy is the failure mode to avoid.
- **Inspector follow-cam.** The camera soft-follows the walking character; whatever he looks at gets
  a bright highlight box and force-opens the relevant overlay — you cannot hide anything, so you
  build *for* the camera's route. Render his POV for ten seconds before arrival so the stealth
  fantasy lands.
- **Compliance artifacts are physical.** Wall plaques you must **hang**; a missing one makes him tilt
  his head and leave a red tag. Visual clutter or a missing log server is a finding — and defenses
  must not *look* aggressive to the auditor's taste (a visible "we read your mail" box fails privacy
  checks).
...
- **"The Colocation Carve-Out" — the goal shifts to leasing space.** Half-racks vs full racks vs
  cages, power-dense vs cross-connect-heavy tenants, NRC/MRC negotiations. Your "customers" are
  other hosting companies — **your competitors** — which makes the whole level enemy-of-my-enemy
  revenue.
- **"The Colocation Landlord."** Customers roll in cages of their own gear and **bring THEIR threats
  to your doorstep**; you defend only the facility layer (power, cooling, physical, network). The
  signature decision: when a tenant's compromised box floods the DC switch fabric, do you cut their
  cage (a contract penalty) or protect the commons?
- **"The Cheap Colocation Landlord."** You now ARE the datacenter — tenants rack hardware in YOUR
  building and you defend the building, power, cooling, physical security, and tenants who bring
  their own drama. The player flips fully into the infrastructure-provider role.
...
- **Give "being acquired" a second act.** Phase 1 = the acquirer's **due-diligence waves** (their
  auditors and pentesters visit YOUR map); phase 2 = the forced-transition mechanics (new compliance
  demands, forced rebrand, frozen budget, key staff quitting). Paired with the inverse `The Buyer`
  level below, the campaign gets a merger **arc**, not an event.
- **The constraint-flip framing.** A bigger company buys you mid-campaign and every constraint
  changes at once — teaching that **corporate events are threat vectors** with the same mechanical
  weight as a botnet.
- **Accept or refuse, with endings.** Accept → you become a business unit with mandated margins and
  a parent-brand SLA; refuse → you stay independent without their procurement deals. Multiple
  endings keyed to MRR, churn and reputation thresholds.
- **"Acquisition Target" (play the multiple).** Maximize the *multiple* before the term sheet —
  growth, MRR concentration risk (any one customer over 30% = a valuation haircut), clean churn
  curves. You can decline and keep playing, **with their grudge as a new competitor AI.**
- **"The Buyer's Due Diligence" (inverted).** You are the one being read: a buyer avatar tours your
  estate like The Auditor's inspector, but every defect deducts from your final **valuation
  multiplier** instead of levying a fine. Three hidden skeletons — an undocumented box, an expired
  cert chain, one tenant running `root` since 2016 — sit findable-and-cleanable in your legacy
  stacks.
- **"The Acquisition" as a second-map mechanic.** Inherit their broken build as a *region* — expired
  certs, default passwords, a worm already loose — and their angry customers arrive with a
  **"badwill" debuff** (low patience, high churn). Teaches the transfer/merge mechanics that
  multi-DC play later reuses.
- **"Now You're IT."** Turn the acquired book around in N days; killing bad customers costs revenue
  but restores sanity — cleanup and disposal as first-class verbs.
- **"Acquisition Absorption."** Their ugly infrastructure (legacy mail, unsupported PHP, debt) lands
  on your board with their angry customers; integrate or sunset — **both cost money and both move
  reputation.**
- **Mid-run forced reskin.** An acquisition ports act-1 skills into act-2 difficulty: inherited bad
  nodes you must migrate off while serving the inherited customer base.

### `The Attacker` / "You Are The DDoS"
Attacker-side interludes where the camera sits on the other end of the cable.

**How it works:** Build a botnet tree, pick an attack vector, and try to crack an AI-run hosting
company. Teaches the defense mechanics **by reversal**, and doubles as a cutscene — "here is what is
coming at you next level." **Keep them short**; they are palate cleansers, not a second game.
**Invariant core:** the same six verbs with the goal node inverted — see `Red Team Friday` above,
which is the in-fiction, authorized version of the same camera.

### `The DR Site` (disaster-recovery reversal)
You play the standby, not the primary. Defense *is* offense here.

**How it works:** Your "customers" (failover traffic) only arrive when *another* region falls, so the
goal is **be ready, not busy** — a level with almost no moment-to-moment work and a brutal grading
moment. Standby systems **decay**: config-drift gremlins spawn if you do not drill. The level is the
argument for `Dry Run` (§1.5) made from the inside.
**Interacts with:** §1.5 `Dry Run` variants, §1.12 The Dry Run, §1.11 The Rebuildability stat.

## 1.3 Hosting-type levels — MOVED

This subsection now lives in `02-hosting-types.md`, alongside the variety-engine
pillar (§0.2) and the hosting-type catalogue (§0.3). Cross-references to §1.3
elsewhere in the document remain valid.
...
- **The pivot IS the level (transition levels as an archetype).** Mid-level the company changes
  hosting type: shared→VPS as customers "graduate", web→GPU as a deal lands, clean→regulated by
  passing an audit. **The map physically reconfigures** — the old path shrinks, the new path opens,
  the apartment block visibly re-partitions into a hotel with tiny contractors — old buildables get
  trade-in value, and ***the old threat set doesn't leave.*** Use these as act finales: hardest and
  most satisfying archetype in the catalogue. A "heritage wall" easter egg keeps a framed photo of
  the old building.
  - **shared→VPS** = an income cliff unless you built hypervisors.
  - **VPS→colo** = you stop owning servers and inherit building risk.
  - **One breach can *force* a transition** (you lose the unregulated market).
  - Implemented as campaign branching: end-of-level "term sheet" events retune the whole next
    level's threat and customer catalogs.
- **"Two Books" (run both businesses at once).** The company pivots shared → managed cloud mid-level;
  old revenue decays as legacy plans sunset, new revenue is lumpy because deals close slowly, and
  the player runs both businesses **on one P&L.** The template generalizes to any type change:
  budget→managed, web→game, domestic→offshore.
- **"Two-Hat Tuesday" (the scale-jump gate).** You must run BOTH the old and the new hosting type —
  still selling shared while launching GPU — so **the old tech tree keeps paying the bills** while
  the capital-starved new buildout gets off the ground. Portfolio and cannibalization tradeoffs
  become explicit.
- **"Move Up the Stack."** Scripted shared→VPS, VPS→dedicated, colo→own-DC transitions where
  customers wait in a visible **migration queue** while you build the new tier; stragglers left on
  the abandoned tier churn or get breached.
- **Player-made pivots.** The build palette expands mid-level while a **migration tax** (retraining,
  new threats) spikes — *when* and *what* to pivot into becomes the strategic layer. Ideal as
  act-bridge levels.
- **"The Deprecation Exodus."** Your core product is obsoleted mid-level (a customer-facing platform
  sunsets); old revenue decays and you must pivot capacity to a new service type (convert
  game-server racks to GPU) while keeping the lights on — a late-campaign twist level.
- **The canonical cross-type pivot arc.** dial-up ISP (nostalgia prequel) → shared → reseller/VPS →
  dedicated/colo → managed services/cloud → GPU/AI or regulated enterprise → multi-region. Each
  pivot is a transition level with the old business still running (legacy cash flow funds the new
  buildout), plus explicit cannibalization decisions — do you raise prices on the dying shared base,
  or milk it?
...
- **"The Ne'er-Do-Well Neighbor" — the shared-fate level.** A colocation scenario in which an
  adjacent cage (AI-run) hosts a spammier neighbor than you. Their junk traffic saturates the shared
  uplink and **poisons the shared IP range's reputation** — damage you take without doing anything.
  Player choices: subsidize their fixes, anonymous-tip the blacklisters, sever BGP, or buy their
  cage.
- **The DC-itself-shared variant.** Other tenants' workloads degrade your latency; you file
  complaints, migrate racks, or poach their customers — **the threat axis is partly social**, which
  is the point.
...
- **"The Reseller Garage" (the tutorial end of the same idea).** You start by reselling someone
  else's shared platform at $20/mo wholesale, charging 10 customers $5 each. It teaches the
  **two-funnel core** — customers arrive via a *sales funnel path*, threats arrive via the *network
  path* — with tiny margins and zero infra risk. Then the upstream host has an outage and all your
  customers churn at once. **Lesson: what you don't own can kill you.**
- **"Channel Civil War."** Your reseller/agency channel and direct sales start chasing the same deals
  (registration disputes). Build a **deal-registration policy** or watch commission fights poison
...
### `Two Maps, One Level` — the physical and the logical
Big late-game levels are one map *with a toggle*.

**How it works:** **SERVER ROOM** (racks, power, cooling, cabling, physical intruders) and
**NETWORK** (routes, services, firewalls, remote attackers) are two views of the same board. Threats
exist on one layer or both, and **you cannot win by only watching one screen.** The signature
late-game difficulty device — and the toggle is literally the game's title screen.
**Interacts with:** §1.10 The Double-Header, §1.7 The density/interface lockstep rule.

### `Two-Board Duo Levels`
One level, two boards, one data plane between them.

**How it works:** A public-facing **edge board** and an internal **back-office board** (billing,
staff, backups) linked by a shared data plane. Threats hit the edge; failures in the back office — a
missed payroll, an unpaid invoice — **silently weaken it**. The lesson is that the business layer is
a load-bearing part of the infrastructure, not a scoreboard beside it.
**Interacts with:** §1.10 The Double-Header, §1.12 (business-layer levels).

### `Dual-Direction Pressure Maps`
Later levels stop having a single "path."

**How it works:** Customers arrive via **many edges** — mobile users, API bots, an office VPN — and
so do threats. The player defends a **shape**, not a lane, and must decide **which edges are worth
serving at all**. The first level that uses this is the one where the player stops thinking in
towers and starts thinking in perimeter.

### `Severance` (datacenter divorce)
The merged company splits and you take half.

**How it works:** A **graph-cut puzzle** over shared racks, tangled cross-dependencies and
split-brain replication: physically and logically partition the assets so that neither half drops a
customer during the untangling — while ***both* halves' threat sets apply** for the duration.
**Interacts with:** §1.2 `The Acquisition`, §1.5 `Zero Downtime Migration`, §1.12 Consent to
Assignment.

### `Stack Merge` (the integration level)
Run two incompatible stacks while migrating between them.

**How it works:** Two control panels, two billing schemas, an old VLAN fabric that refuses to die.
**No new threats at all** — a pure operational puzzle. Each legacy box hides dependencies, and
cross-stack auth breaks in creative ways. **Score = downtime-free merges**, which is the only
scoring rule in the game that ignores both revenue and attacks.
...
- **"Launch Day Spike" (the fuller scenario spec).** A customer's product goes viral — Product Hunt,
  an MMO opening pre-registration, a streamer raid. Traffic arrives as a literal tidal wave, a **100×
  ramp over 3–20 minutes**; and the real boss is the **thundering-herd retry storm behind it.**
  Grade on the *goal metric* — percentage of would-be visitors who reached a serving page, zero 5xx
  from the store — never on kills. Wrong answer: throttling (a bounce). Right answer: pre-bought
  headroom, autoscaling, caching.
- **Unlocks and the two-way lesson.** Autoscaling, CDN caching and the "burst pricing" metered-billing
  buildable unlock here. Overprovision and you burn cash on idle capacity; underprovision and the
  spike becomes an outage tweet.
- **Queue and patience.** Customers physically queue at your edge and bounce when the wait exceeds
  their patience timer — you either pre-scale (spend early, maybe waste it) or scale live while
  swarmed. The economics of autoscaling vs capacity reservation, taught in one wave.
- **The legitimate ramp hides the enemy.** Bots ride *inside* the smooth curve as lookalikes:
  precision filtering towers win, a brute firewall wins nothing. Fake signups can turn into abuse
  mid-path, which forces the capacity-build vs screening-build tension.
- **"The Hug" framing.** The legitimate flash crowd **IS** the boss — they aren't attacking, they're
  customers. Absorb 200× without the ugly countermeasures (CAPTCHA walls, rate limits) that convert
  them into churn; the win is keeping p95 latency and bounce under threshold until the spike decays.
- **Burst capacity as marketing.** Serve the spike with under 2% bounce *and* convert a fraction of
  spike visitors into accounts that stick — the spike is free customer acquisition if you survive
  it. Fail, and the tweet with your logo has 40K likes by morning.
- **Reskinned per hosting type.** On the GPU level it is a "model drop" — everyone grabbing compute
  at once, queue timeouts churning, and **your own autoscaler becoming a liability that overspends
  into bankruptcy.**
- **Visual — the PRESS balloon.** A giant floating "PRESS" balloon drifts overhead casting an
  approaching shadow that says exactly *where* — and roughly *when* — the flood lands; shadow size =
  spike magnitude. An economy shock rendered as readable weather.
- **Visual — the Horizon Tide.** The customer wave is visible on the horizon from minute zero, its
  width equal to expected concurrents; threat waves arrive as darker streaks behind it, so the
  player always sees the two fronts converging. **Horizon-read rule:** the scenario must be legible
  in the world silhouette before any UI — mute the game and you can still name the level type from
  the skyline.
- **Visual — the flood gate and the foam.** A literal gate at the market edge lifts at launch and
  pours a golden tsunami while a red threat-swarm (scalpers, bots, press crawlers) mixes in;
  screen-edge glow intensifies with inbound rate, pipes bulge, undersized capacity objects flash a
  "throat" constriction. **The retry storm renders as foam** — a frothy texture riding the back of
  the flood that only clears when origins stabilize — and a cache hit converts a droplet into a
  satisfying *thunk* at the edge kiosk, so hit-ratio is audible.

#### Reactive agency during the spike — *CONFLICTING*
How much the player may do *while* the wave is passing. The decision turns on whether the climax is
a test of preparation or a test of nerve.

**Position A — pre-build, then spectate** *(master, and opencode wave-1)*: you pre-build on a fixed
budget and then can only react within hard limits — master's rule is that **you cannot scale during
the spike for the first 90 seconds** (cold-start timers), because caching is the only answer that
fits inside the clock. The spike is graded on what you decided before it arrived.

**Position B — telegraph it earlier, and keep a narrow reactive channel** *(opencode, wave-2 game
designer)*: "can only react" is un-fun for anyone who guesses wrong once. **Telegraph the hype
across the preceding waves** (press balloons, pre-registration counters, a visible hype meter) so
pre-building is *informed*, and give the player a narrow live channel — throttle dials, one
emergency autoscale pull — so the climax feels like agency rather than spectatorship.
...
- **"Black Friday Weekend" (the full-stack version).** E-commerce peak: traffic 10×, fraud bots 50×,
  a mega-influencer posting your customer's store, three competitors running promos, and a
  booter-service extortion note. **The attack is *economic*** — chargebacks, fake-checkout botnets —
  rather than destructive: a resource-drain level that tests cash more than walls.
- **The under/over-provision trap, stated in season terms.** Under-provision and your customers'
  stores fall over → refund claims plus rage on social. Over-provision and you eat a month of dead
  margin. **The skill is elastic-capacity timing**, not capacity.
- **The negative-margin acquisition wave.** Hundreds of tiny shop sites sign up on massive discounts
  while payment processors tighten fraud screening, so legitimate checkouts start failing — a threat
  you counter with allowlisting work. Survive the week, then face the January churn cliff.
- **Economy-forward variant.** Ad-supported traffic (high value, low patience) and pay-per-burst
  bandwidth contracts (cheap normally, brutal past the commit); **scoring rewards margin, not
  survival.**
- **Pure-capacity variant.** Absorb a 40× surge for ten minutes of real time without the origin
  collapsing — autoscaling if unlocked, manual node-adding under panic if not; the failure mode is
  cascade and thundering herd.
- **The friendly-fire twist.** Your biggest retail customer and your upstream carrier both peak the
  same week, and the correct play can be **paying a competitor or peering partner for scrubbing
  help** — friend-and-foe economics in the middle of your best revenue weekend.
...
- **The landlord framing.** The landlord sells the building; move every customer from Datacenter A to
  Datacenter B. **Old infra decays** — hardware fails on a timer and you cannot build there — so you
  must migrate *ahead of entropy*. Legacy rack decommissions by day X (rent doubles); rollback is
  expensive; every mis-timed cutover bounces a customer group.
- **Business wrinkle.** You can **charge** for managed migration (a revenue event) or offer it free
  (a reputation event). Migration revenue is a legitimate line item and **migration disasters are a
  top churn cause** — both facts should be on the same screen.
- **DNS weight as a placeable tower.** The player shifts DNS weight gradually with a literal slider
  tower and must **defend the replication link while it is open.**
- **Set the TTLs earlier.** Too long = a slow cutover; too short = a TTL storm if you roll back;
  forget to lower the TTL at all and the level becomes survival horror. A self-inflicted-wound
  mechanic with a fuse lit before the level started.
- **The scripted 90-second bottleneck.** Mid-migration the old site's leases expire (forced
  deadlines) and a downtime meter fills whenever any customer's active path is severed; the boss
  twist is the cut-over moment — **a 90-second window where everything routes through one chokepoint
  you must defend.**
- **Split attention as the signature difficulty.** Customers are *walked across* a migration lane
  while **both** boards take threats; both stacks run side by side on half-budget power, and every
  second of customer-visible downtime double-charges reputation.
- **Acquired-a-rival flavour.** Move 400 tenants over a simulated weekend — each tenant migrated is
  revenue, each botched one is churn plus reputation — while **the old platform's defenses decay
  hour by hour** and yours get overloaded. Enterprise-tenant flavour: 300 VMs from a retiring DC in
  one weekend, with each failed cutover eating a chunk of the contract.
- **Sessions don't migrate — tier the move (correction).** The real playbook is **stateless first**
  (cut DNS), **stateful with replication next** (the dual-write window), **long-lived TCP last**
  (you must drop or tunnel sessions, and the tunnel costs latency). "Keep every session alive"
  teaches the wrong lesson unless dual-run is the actual mechanic.
- **The epilogue wave — the straggler tail.** Resolvers that ignored the TTL keep walking to the old
  site for days; the survivors are a fax machine, an embedded device with a hardcoded IP, and a guy
  named Gerhard on ISDN, each a tiny detective puzzle. (See `The Straggler Tail` below, which
  promotes this into its own level.)
- **Visual — the cut-line and the desaturation curve.** The map is divided by a glowing cut-line
  between OLD DC and NEW DC; the old campus desaturates on a slow curve while the new saturates in,
  so **one screenshot tells you the migration percentage.** Two floor plans (amber old, cyan new)
  are joined by a backbone bridge whose cable visibly thickens as you provision it; customers
  walking the bridge are "in-flight" and vulnerable, and killing the old side's power too early
  drops them off the trunk with an angry splat.
- **Visual — snapped arcs.** Each traffic arc wears a small TTL countdown ring (a melting halo); on
  snap it recoils physically and rains tiny dropped-packet confetti, so severity is measurable by
  **how long the confetti falls.**

#### One level, or a two-level act — *CONFLICTING*
Whether "the migration" is a single siege-length level or a pair of levels with one verb each. The
decision turns on how many systems a single level is allowed to teach at once.

**Position A — one level, everything at once** *(master, and opencode wave-1)*: a single
siege-length level that demands dual-live build-outs, entropy decay on the old estate, per-cutover
minigames and active threats simultaneously. Your build starts finished and you cannot pause; the
pressure comes precisely from doing all of it at the same time.

**Position B — split it into an act of two** *(opencode, wave-2 game designer)*: that is three levels
in one. **Act N** = pre-build and sync while serving, with threats kept simple; **Act N+1** = the
cutover gauntlet, where threats spike and *no new building is allowed*. Each beat gets its own verb,
and neither beat has to be diluted to make room for the other.
...
- **"Forensic Weekend" / "Breach Week" — start twenty minutes after the pop.** Half the map is
  compromised (glitching blackened servers, data draining out as glowing green motes toward a threat
  exiting the screen), customers churning live, press ticking. Triage gameplay: **rebuild clean on
  one side while quarantining the other** — isolate, reimage, rotate keys — with evidence preserved
  *while* remediating. The enemy walks **outbound** paths you cannot block without killing service.
  Reframes "defense" as surgery.
- **Reverse tower defense.** Attacker persistence sits inside as **beacon units** quietly shipping
  data out; **detection towers replace destruction towers**; you hunt hop by hop while still serving
  traffic, which teaches blast radius and least privilege *retroactively*.
- **Your cleanup ops are the waves.** You send incident-response units inward and quarantine racks
  (temporarily offline = revenue loss) while the attacker manipulates traffic against you — and
  **the correct play (pull the plug) hurts your score**, which is the honest part.
- **The backdoor billing you by the second.** A live node inside the perimeter runs up a bill; log
  noise hides the real attacker stream; reputation is tanked so customer inflow is down 30%;
  click-to-isolate costs throughput; and rebuilding trust means an **uptime streak** that slowly
  attracts customers back.
- **Day Zero framing.** Backups are *suspect*: isolate, wipe, restore from the **one** backup you
  trust, re-harden under a ransom deadline. **It's a clock, not a wave timer.** Two servers
  encrypted, the DB exfiltrated, the CEO on line one, and only *some* backups verify — the whole
  "security theater vs actual security" lesson delivered as a to-do list.
- **Two truths the scene usually misses (correction).** (1) **When a box is root-owned you don't know
  what else is** — offer *rebuild-from-known-good + rotate everything* (huge, slow, safe) vs
  *targeted purge* (fast, risky, possibly wrong) as an explicit triage dial. (2) **RAM evidence dies
  when you power off** — make "image memory before pulling the plug" a one-shot action that pays off
  later. And make "half the map compromised" **unreliable**: some green-checked hosts are still
  owned. *The paranoia IS the lesson.*
- **GDPR flavour.** The level starts three hours after you got owned: a ransom note, a press inquiry,
  a **72-hour customer-notification clock**, and a remediation build (forensics, patching, comms,
  credit-monitoring offers) while competitors poach.
- **Visual — glitch-grey and the replay.** Corrupted machines sit under a scanline shader and
  infected LEDs pulse violet instead of green; every corrupted box is both a liability (it leaks
  secrets toward the threat edge) and a clue source. The level opens in darkness — half the map
  padlocked grey, alarms strobing — and a **forensic timeline-scrub HUD rewinds the map's visual
  state to show how the attacker pathed in** (teach-by-replay). Discovered weak points keep a
  persistent scar marker.
...
- **"Cert-pocalypse" — the silent automation failure.** Every TLS cert in the estate expires within
  48 hours because **one renewal automation was quietly broken for 90 days** and you only find out
  now. Triage which cert renews first: mail? payments? the admin panel? **ACME rate limits are the
  boss clock** — the one enemy in the level that cannot be out-spent, only sequenced.
...
- **"Load Bank Month" — the scheduled drill level.** You *voluntarily* pull the utility feed to prove
  the gensets carry the hall: a defense-demonstration in which **the enemy is your own maintenance
  evidence.** The ATS transfers, UPS strings sag (aged batteries read nominal on float and fail on
  transfer — "untested = broken", applied to batteries), one genset wet-stacks, and you are running
  a degrading plant **on purpose**, on a countdown the auditors can watch.
  **Success permanently stamps every machine's "last-tested" badge.** Failure is indistinguishable
  from the real thing except that **nobody forgives you, because you asked for it.**
...
- **"The Registrar Heist."** An account takeover at your domain registrar — social-engineering
  "support" is how it really happens. One morning your domains resolve to the attacker's fake
  building and **the customer road literally repaints toward a rival structure mid-level.** The
  recovery minigame is auth re-proof, transfer-lock rebuild and an ICANN-dispute timer, plus the
  **straggler tail** of cached poison records expiring at random.
  *Teaches that your DNS is a third party's login*, and makes "2FA on everything" a visible
  **registrar lock** buildable rather than a lecture.
...
- **"The Neighbor Fire" — collateral disaster.** A fire in the building *next door*. **You don't
  burn.** The fire department de-energizes the block (the utility feed dies mid-wave; ride the
  generator), a water-main burst from the engines floods the manhole your fiber enters through, and
  the street closes so staff can't reach the site (a shift-rota pinch). Customers only ever see
  "unreachable facility" and the rumor mill starts.
  **Lesson: your failure domains include geography you don't control.** Unlocks the route-diversity
  survey and the second-manhole habit.
...
- **"Ransom Night" — the boss form of the disaster-recovery levels.** Ransomware is the boss and
  **you cannot out-damage it**: the win condition is restore-or-negotiate, and the real strategy is
  how many backups you paid for *before* the incident. Foreshadow it in the pre-level loadout shop,
  so players who skipped backups lose and *learn*.
- **"The Ransomware Weekend."** Everything encrypted Friday 5pm; restore to a clean state before
  Monday 9am load hits — and the backups are old or corrupt. **You discover your real RPO under
  pressure**, which is the only way anyone ever discovers it.
- **"Ransomware Siege."** A note arrives with a copy of your customer data; waves now include
  encryption payloads targeting the storage tier. Paying costs cash and reputation; refusing risks a
  **data-dump event** that mass-churns customers.
- **"The Hostage Level."** 90% of customer data is already encrypted and you have eight minutes to
  restore from the one backup set you (hopefully) built last level. If you skipped backups it is
  nearly unsurvivable — **retro-teaching the whole game's lessons in one clock.**
...
- **The RFP mini-encounter.** A 500-rack enterprise prospect enters the funnel mid-level demanding
  price, SLA (99.99%?), compliance certs, dedicated support and IP ranges. Signing them is 40% of
  revenue — **and their penalties are 40% of your risk.** One whale can feed or bankrupt a host.
- **The divert-or-breach moment.** A single enterprise customer is 80% of revenue with a brutal SLA
  and a private-cluster demand; their box fails while you are mid-fight with a botnet. Divert
  engineers and breach the SLA, or let the bot in.
- **"Land the Enterprise Whale" (the deal as a progress bar).** Sub-objectives: pass the security
  questionnaire, survive a proof-of-concept traffic burst, honor a 99.99% SLA, staff a solutions
  engineer. The deal is a progress bar fed by capabilities; competing bidders can be undercut on
  price or outbid on SLA.
- **"The Contract" as a scenario modifier.** A big enterprise logo appears at the map edge offering
  10× MRR with **contract riders that act as hard rules** (sub-50ms, four nines, zero egress).
  Taking it constrains your build for the level; declining changes the threat mix — *they go to your
  rival, who then comes for you.*
- **"Whale Day" (the split-lane design).** One enormous customer at 40% of revenue arrives as a fat
  slow convoy while hundreds of smalls arrive fast and thin; the whale attracts **targeted APT
  spear-phish packet trains.** Protect the whale vs don't neglect the smalls, rendered as two
  literal lanes.
- **"The Whale Leaves" (the renewal).** The level opens with the renewal meeting eight weeks out: win
  it (price concessions vs service investments vs executive courtship, as a negotiation
  mini-encounter) or replace the revenue before it walks. It leaves behind a permanent
  **concentration meter** that nags whenever any one account exceeds 15% of MRR.
- **"Land the Lighthouse" (the marketing economy in one scenario).** The goal is not serving a whale
  but **converting one into a public case study**: three phases — win the deal paperwork gauntlet,
  keep it flawlessly for N months through SLA boss waves, then negotiate logo rights, reference
  calls and a co-written case study. Each granted right **permanently widens the funnel.**
- **"SLA Death Spiral."** You start holding an enterprise contract whose penalties drain cash faster
  than you can afford repairs; recovery requires a renegotiation minigame or emergency capacity
  rental at bad prices.
- **"Design Partner Sprint."** A prestigious customer offers a 70% discount, brutal custom demands
  and roadmap influence in exchange for being your launch story. Hand-build their platform (one-off
  buildables at 3× cost), ship on time, then **productize the custom work into a real SKU branch —
  or let it rot as bespoke maintenance debt.**
- **"The Reference Account."** A famous logo offers a deal *below cost* for 12 months in exchange for
  a case study: take it for the brand asset plus the enterprise unlock, at the price of real cash
  bleed and morale drag — or decline for a clean quarter and one door that never opens. Logo-wall
  economics.
...
- **"The Audit" as a two-phase level.** A build/reorganize window with only ambient traffic, then the
  regulator arrives and walks the facility checking rooms for required artifacts (log retention,
  access reviews, change records, encryption badges, backups). Every visible defect fails a
  checkpoint = a fine; **three fines = fail.** You can hotfix during the walk, but you cannot hide
  noncompliance — only build it away before they arrive. Boss-pacing beat: *"at minute 8 they ask to
  see last night's backup restore test."*
- **The inspector's gaze as a stealth-game light cone.** Anything caught inside accrues red tags, and
  the map becomes a puzzle of **sightlines, not combat**. Before he enters, a dotted line is painted
  on the floor along his path (repainted if you reconfigure), so the sightline puzzle is legible as
  floor geometry and late hotfixes visibly race the dotted line.
- **Findings cost the pipeline, not your life.** Failures don't kill you, they cost **findings**, and
  findings cost the enterprise deal pipeline — the late-game gate is simply *no compliance, no
  whales.* Passing unlocks the **regulated customer class** for the rest of the game.
- **Convenience is scored against you.** Building "convenient but noncompliant" things — shared root
  credentials, an open DB, exposed admin panels, patch currency below threshold, broken
  segmentation — visibly stains the audit report. Buildables include policy docs, access logs and
  jump hosts; threats are the auditor's checks themselves **plus any real incident during the
  window** (one breach = a failed SOC 2 Type II).
- **The evidence-delivery variant.** The map fills with "evidence requests" that must be *delivered*
  to the Compliance Office along clean paths; your paper-trail buildables generate the deliverables.
  Two simultaneous delivery objectives while normal threat waves keep coming.
- **Calm-wave variant.** Periodic "audit waves" arrive with no combat: the screen overlays compliance
  requirements (segmentation lines, encryption badges, log-retention meters) and you must have wired
  it up **before** the auditor packet reaches your core. Fail = fine plus reputation, never death.
- **Visual — the calendar and the clean-room filter.** A giant wall calendar in the HUD corner tears
  off a page each day, and rooms not yet compliant get a red "NON CONFORM" spray tag. During audit
  scenarios the screen gains a **sterile overlay** — grid rulers, dimension callouts, a paper-white
  tint, a floating inspector cursor tagging objects with red sticky notes — so spaghetti cabling
  becomes *visible clutter the inspector points at.*
**See also:** §1.2 `The Auditor` for the fixed-vs-randomized inspector-route dispute and the
"you cannot hotfix the past" evidence-window correction.
...
- **"The Race to the Bottom" (the price line as a live antagonist).** A competitor keeps cutting
  price mid-level and a **rival price line on a chart creeps down**: match it and margins die, hold
  it and customers drip out. Counter-builds are differentiation — a better support tier, free
  migrations, a "we answer humans" badge. *This is exactly how budget shared hosting dies; winners
  either out-scale or exit upmarket.*
- **The $0.99 variant.** An identical product at $0.99/mo, where premiumization is the only winning
  move; the lose condition is **MRR crossing below payroll.**
- **The funded-competitor variant.** A rival at 60% of your price builds a **"Race to the Bottom"
  pressure meter** each week you don't respond. Options: match (margin bleed), differentiate, or
  keep margin and lose volume — and the level is scored by **contribution margin retained, not
  customer count.**
...
- **"One Typo" (the config-push catastrophe).** The Cloudflare-2017 / AWS-S3-2017 archetype:
  **no enemies at all.** A single deploy — a regex, a flag, an ACL line — propagates platform-wide
  and breaks *everything simultaneously.* The gameplay is the rollback race: detection (**your own
  dashboards drop first — *that* is the alert**), blame isolation, and **staged rollout as the
  anti-mechanic**, because the global propagation timer means the fix must outrun its own fleet.
  Distinct from failure-roll levels: this is a deterministic, self-inflicted platform kill where
  **the enemy is the word "all."**
- **"The Self-Inflicted Tsunami" (the marketing own-goal).** You — or a customer — schedule a
  2M-subscriber email blast or a TV ad at 21:00 sharp: a **scheduled** load spike with a literal
  countdown, placed by your own marketing tower. Pre-scale, or don't; cancelling the blast at T-30s
  saves the infrastructure but tanks the customer's contract. **Attraction tools are loaded
  weapons.**
...
- **"The Quiet Week" (the hidden level).** Nothing attacks, traffic grows gently, alarms are off —
  and it is deeply unsettling. Veterans build nothing and hoard cash. **The twist ending reveals it
  was the calm before the biggest coordinated event in the game.** Lore drops via changelog
  screenshots.
- **"The Boring Level."** Calm traffic, no attacks; scored purely on efficiency, tidiness and prep —
  with a hidden achievement for noticing **the ONE anomaly** a subtle threat is hiding. In New
  Game+ it is full of them, weaponizing the paranoia the player has been trained into.
- **"The Week Nothing Happened" (your own idle hands as the threat).** "While I'm in here..." nudges
  tempt click-to-fix actions with **doubled hidden risk**; unmanaged tech debt ticks up during
  peace; and customers start *wondering whether you are alive* — attraction decays without visible
  liveness, so you ship a changelog or plant a blog post. Place one after every 3–4 brutal levels as
  the enforced breather.
- **"The Quiet Level" (optimization variant).** Low threat, high customer flow; the goal is to
  optimize an existing build for **profit margin, not survival**, scored as revenue per dollar of
  upkeep. It teaches that this is an economics game and rewards retro-fitters rather than
  tower-stackers.
...
- **"Peering Point" — the diplomacy level.** A neutral map node where you, two rival hosts and a
  transit provider share a border. Sign peering agreements (**alliance slots that share defense
  coverage**) or fight a shadow war of BGP hijacks. A rare co-op-with-AI structure, and the only
  level where a competitor can become infrastructure.
- **"Peering Dispute" — the inverse scenario.** Your main upstream drops transit over an SLA spat;
  everything reroutes through one degraded path and **latency-sensitive customers bounce in real
  time.** A negotiation minigame bolted to a traffic-engineering puzzle.
...
- **"Not Your Fault" (upstream meltdown).** Your transit provider has a catastrophic BGP withdraw —
  **you did nothing wrong.** Traffic reroutes over your one degraded backup link at a tenth of
  capacity; the carrier NOC is "on it" (a dialogue tower that yields ETA lies you cross-check with
  looking-glass telemetry). Mechanics: triage order for the scarce backup path, status-page honesty
  vs panic, buying emergency transit at ransom prices. **The level unlocks the multihoming branch**,
  and it grades the things you bought "for no reason."
  *Teaches: your availability is a federation of other companies' mistakes.*
- **"The BGP Leak" (a one-event level).** A tiny config typo upstream announces your customer IP
  space more-specifically from Moscow. The whole level is triage: detect via RPKI/RTBH tooling,
  negotiate with transit, customers scream. **Teaches that your network's *reachability* is a
  defense line you don't own.**
...
- **"The Quiet Night Shift."** Almost no traffic; one server starts behaving oddly. A slow-burn
  insider-threat / APT scenario in which **the enemy is already inside and you must find it before
  it acts.** The visual act is built on the *absence* of light — aisle lights and LEDs only, threats
  exclusively stealth types, searchlight cones and alert sound doing the heavy lifting.
- **Breather variant.** A deliberately quiet night with one single subtle threat — a slow
  brute-force, a dying disk discovered by SMART — teaching that the game's tension can come from
  absence. Works as a narrative beat: the calm before.
...
- **"The Greenfield" / "Breaking Ground" — the build-before-revenue level.** Invert the loop: a raw
  warehouse shell, **zero customers**, permits and inspectors as the clock. Transformer lead times
  (months, compressed to waves), conduit fights with the landlord, fiber ducts you don't own, and
  **the "waitlist road"** — pre-sold customers queue *off-map*, growing angrier while you commission
  power. Win by going live before the waitlist disperses.
- **Milestone payments instead of MRR.** Your "revenue" is milestone-based, and **every facility
  tower gets its commissioning cost here**: a generator installed but never load-tested fails when
  needed. The building itself is under attack by physics — rain on open concrete, a crane in wind.
...
- **"Epoch Overflow" — the scheduled doomsday.** A 32-bit timestamp rollover analog: everything
  breaks **on the dot, in an unknown order.** You know the time; you do not know the victims.
  Pre-level hardening choices, mid-level whack-a-mole. Pairs beautifully with the retro/dial-up era
  arc, where the bug was planted in-fiction decades earlier.
...
- **"Free Tier Purgatory."** Thousands of gnats each worth a cent while a single incident costs real
  money; **you may only build cheap, sloppy, high-false-positive defenses**, so you must accept some
  breaches to keep flow moving. A margin-trimming puzzle rather than a defense one.
- **"The Trial Massacre."** A promo lands 10,000 trial signups overnight and **only 2% will ever
  pay**: deliberately degrade service to the 98% (throttle, queue, self-serve docs) while
  white-glove serving the 2% whose telemetry marks them as whales. A **Fairness meter** punishes
  cruelty with review bombs while generosity bankrupts you — the level has no clean answer, which is
  the point.
...
- **"Capacity Cliff" — 97% utilization, no combat.** Every new customer makes *all* customers slower.
  Turn away lucrative traffic (revenue loss) to preserve quality, or overcommit and risk mass-churn
  collapse.
- **Make the curve concrete.** Latency is a **convex function of utilization** — 10% headroom ≈ no
  tax, 90%+ ≈ exponential stretch — so the "cliff" is *felt in the ribbons* before any number says
  it. Churn cascades from ribbon-breaks rather than a random die roll, and the single readable gauge
  is **river viscosity** (reusing the honey→sludge hue rule).
- **The supply-side variant.** Sales keeps landing big accounts faster than you can provision, and
  **procurement lead time** (servers arrive in three weeks) forces you to buy hosting elsewhere
  short-term at bad margins.
...
- **"Split-Brain Sunday" (the first encounter with consistency as gameplay).** Two regions, a
  replicated DB; a fiber cut partitions them and **both sides keep accepting writes.** A forced
  decision under a countdown: pick a winner (lose the other side's writes) or freeze (lose the
  uptime SLA). Then the cut "heals" and you must **reconcile.**
- **"The Sane Distance" (the metro-stretch level).** Two halls 40km apart that should *feel* like
  one: sub-2ms RTT makes stretched L2 tempting and **wrong.** The level teaches cluster-over-
  distance — **quorum arbiter placement** (a third site as tie-breaker; the two-node lie revisited),
  storage replication lag as a visible rubber band, and **the one-fiber truth**: both your "diverse"
  metro routes land in the same duct, and real dual-path is something you *buy*. HFT-lite tenants
  ask you to run *closer*, which turns cable length into a placement decision with microsecond
  accounting.
- **"Two Facilities, One Brain" (the multi-region prequel).** Split-screen primary DC and scrappy
  secondary: customers served from either, threats target both, but **staff and budget are shared**
  — which forces delegation and the automation of one side.
...
- **"The Big CVE" (Patch Rush).** A catastrophic remote-exec vuln drops on day 3 of the level —
  Log4Shell/ProxyLogon class. **Every buildable containing the vulnerable component glows red** and
  exploit waves begin within minutes. Patch in dependency order while customers scream about
  downtime windows: patching reboots = downtime, not patching = a breach timer.
- **"The SSLpocalypse" variant.** A certificate-authority expiry or Heartbleed-class event: patch or
  replace every cert before browser warnings mass-churn your customers. Its real subject is
  **inventory and asset management** — you cannot fix what you never listed.
...
- **"Blocklist Purgatory" — the reputation meter *is* the health bar.** The level starts one mistake
  deep: a tenant ran an open relay before you had abuse detection. A pure reputation game — warming
  IPs, feedback loops, spam complaints as damage, **delisting requests as boss fights.** Very few
  buildings, a great deal of mail.
- **Reach-the-inbox variant.** Build reputation while spammers on your netblock try to sink your IP
  into RBLs; **reputation is a hidden stat that gates revenue**, which makes it the most frustrating
  and most memorable meter in the game.
...
- **"Chargeback Storm" — defend the bank account.** A pricing mistake or a competitor smear triggers
  mass chargebacks and cancellations. **The lose timer is money, not uptime**, and the towers are
  PR, support quality and refund policy.
- **The fraud-ring flavour.** A ring signs up on trial cards and every invoice reverses *with a fee*
  — cash bleeds while you are still powering the servers. Counters: card AVS checks, an
  identity-verification buildable, deposit-required plans. **Lesson: cheap customer acquisition can
  be negative revenue.**
- **Make the metaphorical towers literal.** A **Dispute-Response desk** (each chargeback arrives as a
  card; staffing it in time converts ~60% back), a **Processor-Health meter** (let disputes rise and
  the processor freezes your account — a second, worse timer), and a pre-level **Refund-Policy
  Dial** that determines wave composition. Lose = the cash floor; win = dispute ratio under
  threshold at the timer.
...
- **"Payroll Valley" — the calendar is the enemy.** No boss at all: biweekly payroll lands *before*
  the monthly billing cycle completes, and transit and power invoices stagger differently. Survive
  six paydays without a collections event while the growth target ticks. **Teaches the operator's
  first real truth: profit on the books, dead in the bank.**
- **"Bootstrapped Winter" (hardcore economy).** No funding, six months of runway, every dollar real.
  Learn **float**: annual prepaid plans give cash today, hardware leases smooth capex, net-30
  invoices from enterprise customers hurt. A soft *runway* mechanic replaces a countdown timer.
  *Variant:* $500 starting capital, where every dollar is power or bandwidth, reselling and
  overselling are forced, and the lose condition is bankruptcy.
- **"Bankruptcy Watch" (permadeath-lite).** If cash drops below one week of opex a countdown begins
  and creditors — bandwidth bills, payroll, hardware leases — **foreclose on buildables one by
  one.** Survive via emergency invoice factoring (cash now, a discount later) or a bridge investor.
- **"Chapter 11: The Comeback Level."** A designed loss-*avoidance* scenario: you start insolvent,
  with creditors, a court-approved plan, **critical-supplier agreements that keep your upstream from
  dropping you during restructuring**, and a 12-month exit window. The only level in the game where
  **firing your worst customers is mandatory** to make plan.
...
- **"Vendor Squeeze" (the COGS shock).** Mid-level your upstream licensor gets acquired and re-prices
  you — a cPanel/VMware-shaped 3–5× increase at renewal. Absorb it (margin dies), pass it through
  (a churn spike), or **migrate the fleet to open-source equivalents** (support load explodes during
  the transition). One brutal, extremely recognizable gross-margin puzzle.
- **"The Power Spike" (a Texas-style grid event).** Wholesale electricity goes 100× for 48 hours:
  colo and GPU operators **without pass-through clauses eat it**; *with* them, customers get shock
  invoices and churn anyway. Hedged power contracts (PPAs) unlock as the defense — which is to say
  the defense had to be bought a year earlier.
...
- **"The RFP Mill" (pipeline math as a recurring scenario).** Every quarter 1–3 public or enterprise
  RFPs land. Each costs real staff-hours to bid (an explicit **bid/no-bid decision**), pays off
  probability-weighted weeks later, and the winner's contract carries penalties negotiated under
  time pressure. **Too few bids starves growth; too many eats losses on the losers.**
- **"The Tender Freeze" (government hosting).** Procurement reform: budget sequestration **freezes
  all contract awards mid-pipeline** — your enterprise pipeline towers go dark and only sticky MRR
  and consumer lines keep cash flowing. Government demand is high-margin but politically fragile,
  and this is the level that proves it.
...
- **"The Bankruptcy Auction."** A major rival fails mid-campaign and you bid real cash against three
  competitor AIs at $2/account. **Winners get the revenue AND the landmines**: half the book is
  abuse tenants, every migration is a support tsunami, and you also inherit the goodwill of the dead
  host's community. *The fastest money lesson in the game: buying churn in disguise.*
- **"The Portfolio Buy."** Buy a failing competitor's customer book at 2.5× ARR and inherit hidden
  churn cohorts, EOL hardware running the load, legacy pricing that cannot survive re-platforming,
  and two abuse complaints you didn't cause. A mini-diligence phase reveals *sampled* landmines, and
  purchase-price amortization rides along as cost.
- **"Competitor Meltdown" (the harvest scenario).** A rival's catastrophic outage opens a winback
  window: their customers flood your funnel and sales calls multiply 10× — but **onboarding faster
  than capacity creates your OWN quality collapse**, and the temptation is signing abuse-prone
  refugees because they're desperate.
- **Buying the graveyard.** Their infrastructure is snowflake servers, undocumented cron jobs,
  expired certs and 4,000 angry customers: stabilize, consolidate, migrate — **while *their* old
  attacks keep coming to you**, because customers blame the new owner.
- **Between-levels market event framing.** A failing competitor puts their customer base up for sale
  as a market event: buy and inherit BOTH the revenue AND their unpatched vulnerabilities, abusive
  tenants and bad reputation. Run a **due-diligence mini-scan** first.
- **"The Buyer" (inverted acquisition).** Pay cash and inherit their customer book (with churn debt)
  and their tech-debt inventory — **their gremlins spawn on *your* map** — plus one prized node you
  actually wanted. A 10-wave "integration window" runs before synergies unlock, and selling assets
  too fast walks the whales.
...
- **"IPv4 Famine" — the address as the scarce crystal.** No new address space: every new customer
  needs multiplexing — CGNAT, shared IPs with SNI, port-forward gymnastics. A capacity puzzle with a
  late escape hatch (secondary-market address purchase at brutal cost), and the nasty coupling that
  **shared IPs poison each other**, email especially.
- **"Dual-Stack Decade" (IPv6 as a scenario).** Mobile customers **arrive IPv6-only**, spawning at
  the road's far end with no v4 on-ramp, while legacy customers only come v4. You must run
  dual-stack, and translation gateways (NAT64/DNS64 towers) are **lossy** — they bounce some and
  misroute others. *"IPv4 Famine" is the crisis; this is the boring decade-long rebuild* — more
  honest, and better as a puzzle.
...
- **"The Landlord Squeeze" (the colo divorce).** Mid-campaign your colo provider raises power and
  space prices **40% at renewal**: absorb it (margin bleed), pass it through (a price-hike churn
  wave), or **migrate half the estate** using the migration level's mechanics on a schedule you
  choose. *Colocation means your worst outage can be a PDF from accounting.*
...
- **"The Lift Move" — the migration level's evil twin.** Not packets: **racks.** The landlord sells;
  move 40 racks to a new hall 3km away across six weekends. Each rack moves as **one indivisible
  chunk** — freight-elevator weight limit, dock height and ramp angle become literal placement
  puzzles — and a moved rack's service stays dark until it lands and re-converges, with DNS TTL
  countdowns racing the truck. Split the fleet (a stretched cluster, quorum anxiety) or take planned
  downtime windows. **A dropped rack means a customer's irreplaceable legacy box physically dies**
  (an insurance-claim event). *Nobody has ever put a freight elevator in a tower defense.*
...
- **"Hurricane Season" — the twenty-minute forecast you can only partially trust.** A storm track
  crosses toward your coastal region; the choices are raise gear off floor tiles, seal the facility,
  buy generator fuel (truck deliveries), pre-warm a backup region — or gamble. **Grid power dies on
  a timer**, and attack waves opportunistically hit *while* you deal with the storm, because
  attackers watch your status page too. The real defense is against your own failure cascade.
- **The mercy mechanic: customers forgive outages *if you communicated*.** This adds an announcement
  ability, and it is the seed of the whole disclosure-honesty system (see `The RFO` below).
  Customers with their own DR plans don't churn; ones without do — and they blame YOU publicly.
- **Demand pricing under a storm.** Raise prices on surge — profitable but reputation-toxic — and
  decide which customers to warn. Operators do exactly this calculus before a storm.
- **No smart attackers at all (variant).** Grid, cooling, flood gates and fuel degrade on a weather
  clock; **generator fuel is the new ammo**; customers churn by the minute as latency climbs; win =
  hold N nines through a 30-minute storm.
- **The physical plant *is* the tower roster.** A generator start-up race, flood-barrier placement, a
  fuel-delivery convoy you must escort on a side road, a cooling intake taking on water. One utility
  pole goes, a fuel delivery is blocked by flooding, and **N+1 everything turns out to be a
  *spiritual* exercise.**
- **Disaster recovery as a purchase decision made before the disaster.** Fuel the generators early
  (cost), pre-warm failover (cost), ride it out (risk), or preemptively declare a degraded SLA
  (reputation cost, credits avoided). Economics invert: uptime costs fuel dollars and **you choose
  which customers to brown out.**
- **CDN flavour.** Keep the global SLA while 2 of 5 POPs brown out — multi-region redundancy, load
  shedding, and generator fuel as consumables.
- **Add the physics (correction).** The genset cannot carry full load *and* cooling once grid and
  water are both gone: real evacuation events die of **wet-bulb**, not flood, so a temperature +
  humidity meter over the building decides whether you can carry peak. And make **"exercise the
  generator monthly"** a scheduled mini-task with a visible last-tested stamp — players who skip it
  get the real 2017 hurricane-DC outcome: *it cranks and cranks and doesn't take load.*
- **Visual — the forecast cone and the river gauge.** The cone renders as translucent hazard paint
  with **confidence blur on its edge** (sharp near-term, feathered far-term); power lines crossing
  the cone sway and flicker early as the diegetic tell. Weather renders over the *building* while
  the interior stays crisp — rain sheeting on windows, lightning freeze-frames that reveal the room
  in negative, a waterline climbing the loading dock, generators sputtering with visible exhaust,
  repair crews as tiny hi-vis figures. In colo levels where you don't control the building, **the
  screen shakes and you can only react.** A river-gauge post with historical crest marks charts your
  trauma across the campaign, and the communication mercy gets a **radio-announcement tower** whose
  expanding ripple rings wash over approaching customers and re-colour their ribbons green.
...
- **"Sustained DDoS Siege."** Not a wave — a **30-minute relentless volumetric attack with varying
  vectors**, survived while one specific high-value customer keeps their SLA. Introduces scrubbing
  and upstream blackholing as mechanics, and it is the level that teaches the difference between
...
- **"Famous The Wrong Way."** You onboarded a controversial streamer or game, and a booter service
  now advertises "breaking you" on its public dashboard: sustained low-grade DDoS for weeks,
  per-strike extortion, and copycat attackers. **This is the level that introduces the extortion
  threat family** — and the first time the player's customer list is itself a liability.
- **"Rival Host's PMUD."** A competitor runs a coordinated smear plus attack, with **DDoS waves timed
  to coincide with your outages so *their* status page looks better.** Introduces the
  status-page-as-health-bar mechanic.
- **The multi-act rival narrative.** A rival repeatedly leaks your customer list (a phishing wave),
  buys your upstream transit (a latency nerf), and poaches your NOC staff mid-level (staff towers
  strike) — **each act teaching a different vulnerability axis** rather than the same one louder.
...
- **"Endgame: The Exit."** The final campaign level is scored on company **valuation**: an ARR
  multiple modified by growth, churn, concentration, margin and brand, optimized before a
  trade-sale/auction event. Endings: strategic acquisition, PE rollup, IPO, or *"we sold the
  customer list and kept the domain."*
- **"The Rule of 40" (endgame grading).** You are scored on **growth% + EBITDA margin% ≥ 40**,
  reachable either by hypergrowth (burn, land-rush, marketing blitz) or by profitability (cull
  products, raise prices, shrink support). It sets the meta-goal for endless mode, and it is the
  only scoring rule that treats two opposite strategies as equally correct.
- **"IPO Sprint."** Target 40% YoY growth, clean books, and **an uptime history that survives
  diligence**, while a short-seller publishes a hit piece and an activist demands cost cuts. Every
  decision is scored twice: cash now vs the valuation multiple at the buzzer. As a late campaign
  milestone, passing it permanently spikes visitor volume — and adds a **stock-price meter that
  punishes downtime far more harshly than before.**
- **"Series A / Growth Capital" (the meta-level).** Investors offer $5M for 20% and a **growth
  covenant** — triple ARR in 18 months or the board replaces you as the player-character. The whole
  level plays under forced-spend pressure, with every decision judged by growth rate *and* burn
  multiple.
- **"The Down Round."** Growth stalls and the market turns: accept less money at a lower valuation
  (preferred-conversion drama, a morale hit carrying staff-attrition risk) or refuse and cut burn to
  profitability. **Capital is a customer-acquisition weapon with strings.**
- **"The Long Tail" (endgame meta-progression).** After "winning", an endless franchise mode: a
  portfolio of hosting lines, board members demanding dividends, and **Black Swan cards** — a
  pandemic is a remote-work boom for gaming and VCU, AI hype is a GPU gold rush, crypto winter is
  the abuse wave receding.
...
- **"Dial-Up Days" / "Dial-Up Time Capsule" — the retro breather.** 1996: modem banks, ISDN,
  per-minute billing, sysops, `ls -l` pride. **Total bandwidth is fixed and tiny**, so the level is a
  queue-management rhythm game where busy signals are bounced customers. Threats: busy-signal spam,
  war-dialers, mom picking up the phone, and one fax machine causing chaos. A low-stakes,
  high-charm aesthetic palate cleanser between industrial-scale levels.
- **The bottleneck is the tower slot itself.** Customers literally dial in through a limited bank of
  modem ports, with modem whine as ambient audio. **The lesson is that queueing is lossy** — the
  same lesson the rest of the campaign teaches at 10,000× the scale.
- **Interlude flashback framing.** Play the company's founding on a 1996 dial-up BBS — 56k modems,
  busy signals, SysOp lore, threats limited to phone-line tampering and one smug hacker in a garage:
  a palette swap of the tutorial mechanics.
- **"Y2K Retro Campaign" (alternate whole campaign).** An entire campaign set in 1999 — dial-up ISP
  scale progression, no cloud, no TLS, and a "firewall" that is a second NIC in an old PC. Modern
  mechanics with period-authentic limitations.
...
- **"The Offboarding Level" — the goal is to *leave*.** The contract ended: migrate all customers
  OFF cleanly within budget while maintaining service, or pay breach penalties. **It reverses the
  whole game and tests everything you learned** — an endgame capstone that scores you on how
  gracefully you can stop.
- **"End of Support" — the calendar as the enemy.** Vendors stop patching your stack: old PHP, old
  kernels, Windows 2012 tenants who *refuse to move*. Every new threat does extra damage to EOL
  boxes, and they visibly **lack the patch-glow shield**. The decision tree is force-migrate (churn
  risk), run exposed (segment and hope), or buy the absurd extended-security-updates tower (a money
  sink that buys one level's truce). Pairs with `Legacy Debt`, but where that level's enemy is mess,
  this one's is *time*.
...
- **"The Outage Post-Mortem."** The level starts two hours after a catastrophic outage: the ticket
  queue is exploding, X is on fire, SLA credits are due, and two customers have already churned.
  Rebuild trust under time pressure with a **status-page honesty meter**, compensation offers (cost
  cash, buy reputation) and PR buildables.
- **"The Aftermath" (the apology tour).** After a famous six-hour outage: SLA-credit payments
  draining cash, a churn cascade mid-walk, competitors' FUD vans circling, a regulator requesting
  your post-mortem — and **the honesty dial**: publish the real cause (a short-term reputation hit,
  a long-term trust buff) or spin it (an immediate buff, then a delayed discovery event that hits 3×
  harder). Communication as a first-class defense; the hurricane's mercy mechanic promoted into a
  full scenario.
- **"Roast Day" (the public post-mortem as a minigame).** Drag root causes onto a blameless template
  under dialogue/timing pressure: honest disclosure = a temporary press dip then a **structural
  attraction buff**; a cover-up = no dip now, then a "leak" threat wave later, with a doxxing
  variant.
- **"Breach & Confess."** Contain, remediate, then **communicate**: write the disclosure and choose
  how honest the postmortem is. Honest-but-fast costs cash and builds long-term trust; the cover-up
  is cheap now and catastrophic later — threat cards gain "leaked breach" variants for the rest of
  the run.
...
### `The Two-Node Lie`
The HA intro level. **Archetype: Reach-State. Length: Interlude.**

**How it works:** The objective is to survive a full server failure with **zero downtime.** The level
teaches that **a load balancer with one backend is just a single point of failure with extra
latency**: if the player builds LB + 1 web + 1 DB, the DB death event wipes them and the lesson is
delivered by the corpse rather than by a tooltip.
**Interacts with:** §1.5 `Two Masters` / `Split Brain` (the quorum arbiter is the sequel to this
lesson), §1.1 Tier 2.5 (failure domains).

### `The Bill Comes Due`
A massive bandwidth-overage invoice lands mid-level. **Archetype: Negotiate. Length: Scenario.**

**How it works:** Reach a cash threshold before the payment timer expires, or the upstream pulls your
uplink. It forces monetizing customers aggressively against spending on defense — the purest
statement of the game's money-vs-safety tension.
**Spec it as three buttons, each a system:** **pay in full** (safe) · **pay partial** (spawns
"late-fee" interest units that compound on the money meter) · **a terms-negotiation minigame** (a
time-limited dialogue with the upstream; success buys 15 days and one wave of breathing room,
failure is the uplink-pull). **Tie the invoice amount to the live upkeep ledger so the player
*already knows* the number** — dramatic irony, not surprise.

#### Unpaid-upstream-bill outcome: hard lose vs soft deplatform — *CONFLICTING*
What happens when the bill is not paid. The decision turns on whether commercial failure should be
modelled as an instant kill or as a relationship that decays.

**Position A — the hard lose** *(opencode, general)*: reach the cash threshold before the timer
expires, or the upstream pulls your uplink; the three-button spec keeps failure as the uplink-pull
loss animation. Clean, legible, and the money meter becomes a real health bar.

**Position B — the soft deplatform** *(opencode, wave-2 CEO)*: an instant uplink-pull is arcade;
real deplatforming is **soft** — latency deprioritization, "maintenance" windows that favour other
customers, no renewal at term. Replace the hard lose with an **upstream patience meter** that
degrades with late bills. It makes the game's own peering and carrier-relationship unlocks
load-bearing, and turns multi-homing into redundancy against **commercial** failure, not just packet
failure.

### `Crypto Winter`
The demand-collapse level. **Archetype: Endure. Length: Siege.**

**How it works:** GPU customers vanish one by one, prices crater, **upkeep stays.** Survive N months
without bankruptcy while pivoting — repurpose the GPU fleet to rendering or backups. It is the level
whose entire thesis is that **overbuilding is itself a threat**, which no amount of defensive play
protects you from.
**Interacts with:** §1.5 `The GPU Gold Rush` (this is its hangover), §1.12 (capex and depreciation).

### `The Long Night`
The endurance level. **Archetype: Endure. Length: Siege.**

**How it works:** No build phases, limited repair resources, and an **on-call fatigue mechanic.**
Pure operational triage: **you win by *not* making a mistake at 4am.**

#### Endurance structure — *CONFLICTING*
How endurance is actually produced. The decision turns on whether length or scarcity creates the
feeling of a long night.

**Position A — 100 waves, no build phases** *(opencode, wave-1 sysadmin)*: a literal marathon. One
hundred waves, no respite, limited repairs; the exhaustion is real because the runtime is real, and
the level earns its name by outlasting the player's concentration.

**Position B — a dozen scripted incidents and a repair budget** *(opencode, wave-2 game designer)*:
100 waves is pacing suicide — nothing in the game earns a 40-minute reflex grind. Replace it with
**~10–12 scripted incidents at escalating severity**, a repair-budget pool, an on-call fatigue
meter, and **quiet windows whose only decision is sleep-or-patrol**: endurance through *stakes and
scarcity*, not unit count. Rebrand the level accordingly.

### `Legacy Debt` / `Legacy Purge`
Inherited messy infrastructure that works right up until you touch it. **Archetype: Diagnose.
Length: Siege.**

**How it works:** A Cobol box, a mainframe configurable only from its own console, undocumented
VLANs. It *works* — but every action risks an unexplained failure. The tension is clean-rebuild vs
keep-running, and neither is free.
**Add the two classic folklores mechanically:** (a) a dependency marked **DO-NOT-DELETE** that turns
out to be the build cache, and (b) a dependency with **no label at all** that turns out to be
load-bearing. **The only one that ever kills you is the undocumented one** — run the discovery scan
or eat it.
**Companion: `Legacy Purge`** — decommission ancient hardware that still serves stubborn customers.
Retiring a box plays a "boxed up with a memory frame" animation; some customers grieve and walk
slower; there is a score bonus for **zero snapped dependency cables.**
**The Legacy Tax (the carry-over rule):** early cheap or shortcut decisions accumulate as permanent
legacy debt that charges ongoing **upkeep** until deliberately refactored — and `Legacy Purge` is its
resolution path, so the tax has an exit rather than being pure punishment.
**Cross-act version:** whatever you leave running at the end of a level carries into the next as
"legacy systems" — free capacity, but un-upgradeable and **a beacon for matching exploit threats.**
Encourages deliberate cruft; punishes accidental cruft.
**New Game+ flavour:** replay the campaign keeping **one bad decision** from your last playthrough
(a compromised server, a lawsuit) as permanent scar tissue that periodically re-activates.
**Interacts with:** §1.6 Legacy debt carry-over, §1.5 `End of Support`, §1.11 The Rebuildability
stat.

### `The Offshore Gamble` / `The Chain of Custody` / `The Subpoena Raid`
Bulletproof money pays 3× better, and every shady tenant raises a **seizure meter.**
**Archetype: Endure/Negotiate. Length: Siege.**

**How it works:** At full meter a law-enforcement raid wave hits, and **you cannot legally counter
some threats.** The raid is an **unstoppable unit you only prepare for** — sealed drives,
jurisdiction routing, a counsel tower, data-migration speed — and **the loss condition is losing
specific evidence, not money.** The placement mechanic is *which jurisdiction hosts what.* Morally
gray by design, because the player chose this market.
**The raid as a real-time choice:** comply (lose the customer base, keep the license), stall with
lawyers (cost, uncertainty), or run (the offshore path — legitimate-partner reputation dies, street
cred soars).
**Seizure Watch:** the Heat meter climbs with every abusive customer; jurisdiction switching moves
**the same servers** onto a different country map and the legal threats re-form differently;
takedown notices tick as a slow countdown. The terrifying event is the **lights-out seizure**, where
an entire cage vanishes overnight.
**Variants:** *"The Offshore Detour"* — a shady contract that pays 3× but summons law-enforcement
pressure and a bad-PR meter you must outrun or legitimize away later. *"Offshore Cold Shoulder"* —
rivals' DDoS, your payment processor pulling the merchant account, and an Interpol seizure request
that is **"a win you can lose by having the wrong paperwork."** *"Protest / Deplatforming"* — a viral
controversy sends both a legitimate tsunami of curious visitors who won't stay and an
"Anonymous"-style attack coalition; you may publicly drop the controversial customer for a
reputation swing in either direction, **and that moral choice decides which threat table activates
next wave.**
**Hidden "Grey Market" unlock:** after accepting a sketchy customer three times, a level opens where
law enforcement is unblockable (only delayable via jurisdiction layers) AND **your customers churn
*upward* to normal providers if you get too good at uptime** — an anti-optimization joke level that
is genuinely a great puzzle.
**Interacts with:** §1.12 Offshore, §1.5 `Sanctions Screening`, §1.11 (Franchise Map jurisdiction
cards).

### `Escorting a Container` / `The Slug`
A payload-escort wave grafted onto the build game. **Archetype: Protect. Length: Interlude.**

**How it works:** A customer's single huge workload — one 40U GPU sled — must be **physically driven
across the map** from truck to rack while under attack.
**"The Slug" variant:** escort one 800 kg legacy tape library across a hostile city by truck. Its
fragility, weight and power needs **change as you transfer it between vehicles** (an air-ride
suspension buildable, an anti-static padding upgrade), and if it dies, **your biggest customer's
archive is gone permanently.**

### `The Cutting-Over Gantt`
A deadline scenario whose entire UI is one bar. **Archetype: Reach-State. Length: Scenario.**

**How it works:** A rendered timeline floats at the bottom of the screen and **all of the level's
work must finish before the marker sweeps to "GO LIVE"** — construction crews visibly race the line.
The Gantt is not a HUD element bolted on; it is the level's only clock, its only telegraph, and its
score sheet.
**Interacts with:** §1.10 The Objective Totem, §1.5 `Datacenter Build` / `First Watt`.

### `Everything Rains` (Murphy's week)
The gauntlet level, and the default shape of late-game pacing. **Archetype: Endure. Length: Siege.**

**How it works:** **5–7 *medium* unrelated failures inside one shift** — the flaky PSU AND a cert
expiry AND the junior's bad deploy AND a fiber cut in the same block. The teaching mechanic is
**triage ordering under a "second-worst" clock**: fixing things in the wrong order cascades.
**The real ops law underneath it:** failures cluster *because they share causes* (storm → power →
cooling → panic-deploy), which generalizes the "boss design = stacks" idea into the **default**
late-game pacing rather than a special event.
**Interacts with:** §1.14 `The Cascade`, §1.7 The Pressure Budget.

### `The Straggler Tail`
The migration that won't end. **Archetype: Diagnose. Length: Interlude.**

**How it works:** Sequel grammar to *any* migration level: 97% cut over cleanly, and the remaining 3%
are customers whose resolvers ignored the TTL, a fax machine, an embedded device with a hardcoded
IP, and a guy named Gerhard on ISDN — **each survivor a tiny detective puzzle.** It captures the
actual feeling of a migration: the long, humiliating tail.
**Interacts with:** §1.5 `Zero Downtime Migration`, `The Registrar Heist` (cached poison records are
the same shape).

### `The Maintenance Collision`
Two planned events overlap, and every individual action was correct. **Archetype: Diagnose. Length:
Scenario.**

**How it works:** Your upstream carrier's "4-hour window, 02:00–06:00, no exceptions" lands the same
night as your own cutover **and** a neighbouring colo's power works — with **fog of war on the
calendar.** It is a meta-layer scheduling puzzle: you see only your own windows, and NPC notices
arrive late or as form letters. **The trap is that no single decision was wrong — the collision is a
fault class no log shows.** Maintenance windows shield you from SLA credits; they do not shield you
from *each other*.
**Interacts with:** §1.10 The Change Window, §1.5 `The Change Freeze`.

### `The Subsea Severance` / `Cable-Cut Winter`
Three submarine cables cut within a day. **Archetype: Convert/Endure. Length: Siege, or an arc.**

**How it works:** A fishing trawler, an anchor drag, and a "mystery." The world's traffic reroutes and
**your landfall path becomes the last healthy lane**: everyone else's customers flood toward you
(hug-of-death ×5), transit spot prices go 100×, and a **repair-ship minigame** opens (charter a
cable ship, a days-long cooldown, cost-shareable with other operators). **Gouge or be a hero** — the
pricing fork is a reputation choice with a long memory.
**Arc form:** three cuts in one month **reshape the world map for several levels** — persistent
global latency inflation, reroute puzzles through long detours or absurd satellite lanes, and
customers with latency budgets churning *by geography*. The arc ends when the repairs complete,
teaching in the most expensive way available that **the internet is physical.**

### `Space Weather`
A geomagnetic storm degrades three normally-invisible systems at once. **Archetype: Diagnose.
Length: Scenario.**

**How it works:** Power frequency, NTP accuracy and satellite backhaul all degrade on **one shared
storm-forecast clock with noisy forecasts.** Induction-voltage events pop transformers in a
geographic band, so your spares strategy finally pays off; and **clock drift silently breaks auth,
logs and consensus before anyone "sees" an attack.** An NTP stratum-1 source becomes a hero
buildable — the least glamorous object in the catalogue, promoted for one level.

### `The Deep Test` (the sealed capsule)
A sealed undersea habitat capsule must serve 90 days with **zero physical access.** **Archetype:
Endure. Length: Siege.**

**How it works:** Every repair is software-only. A pre-launch checklist phase (build quality vs mass)
decides everything that follows; condensation and marine-growth meters tick on a hull overlay; and
your only comms cable starts subtly flapping — *storm, or shark?* **Remote-hands economics inverted
to zero.** Unlocks the "sealed hardware" doctrine. A late-game palate cleanser.

### `Light-Year Latency`
Host a deep-space or HPC-remote site 600ms+ RTT from the home NOC. **Archetype: Tune. Length:
Scenario.**

**How it works:** **Your own inputs are delayed** — orders land half a second later — so you must
**pre-authorize policies** ("if X then Y while I sleep"). Incident command becomes *writing rules
instead of clicking*, and handshake-heavy threats (mimics) get a home-field advantage. **The
automation ladder is the only answer**, which makes this the level that justifies every automation
purchase the player resented.
**Interacts with:** §1.2 `The Apprentice` (you may only write rules), §1.11 Ratchet Progression.

### `Harvest Now, Decrypt Later`
A campaign arc where the threat is a countdown no tower can touch. **Archetype: Endure. Length: an
arc across 3–4 levels.**

**How it works:** For three or four levels **almost nothing bad happens** — except a quiet APT
variant that only *mirrors* your encrypted traffic. A countdown runs behind the campaign. In the
finale they "activate their quantum machine" and **every stored session replays as live attacks**
unless you rotated to post-quantum crypto in time. Cryptographic agility, sold as insurance and
collected as a bill.

### `The Oversell Doctrine`
Shared-hosting economics as a level. **Archetype: Tune. Length: Siege.**

**How it works:** You can host 500 sites on one machine ***if nothing goes wrong***. An **Oversell
Ratio** meter is the dial: push it for margin, push too far and noisy-neighbor events chain. Lose
condition is reputation collapse; win is a quarter with positive cash and the SLA intact.
**"Shared Server Graveyard" variant:** one overloaded box, 300 sites, support tickets as ambient
noise — **every new customer raises density and meltdown risk**, teaching the
density/revenue/instability triangle directly.
**Interacts with:** §1.6 Authentic difficulty knobs (oversubscription ratio is named there as the
most honest dial in hosting).

### `The Esports Finals` / `Game Launch Night`
Tentpole levels for game hosting. **Archetype: Convert. Length: Scenario.**

**How it works:** A tournament on your platform draws 10× concurrent players for six hours,
live-streamed. **Win = zero lag spikes on featured matches.** A streamer saying "these servers are
butter" spawns a fanwave of new players down the funnel path; one exploitable cheat or crash on the
main stage triggers a review-bomb instead.
**"Game Launch Night" variant:** a studio's MMO goes live at 100k concurrent at T=0, and **the auth
queue is your primary defense** — queue-jumpers and DDoS-by-hype ride the same surge. Win = a
"launch weekend success" meter.
**Capacity-engineering framing:** the level is won by pre-built queues, autoscaling policies and
shedding systems while hype-scrapers and a competitor's DDoS ride the legitimate curve. In its
simplest form — *"The Launch"* — you must provision enough capacity BEFORE the wave, against one
monstrous hockey-stick: overshoot drains upkeep, undershoot mass-bounces.

### `The GPU Gold Rush`
The AI-bubble level, and the only one you can win and still lose. **Archetype: Convert. Length:
Siege.**

**How it works:** A wave of lucrative AI tenants wants 40× A-class nodes **now**: capex is brutal,
spot revenue is juicier, and **every GPU node you light attracts miner abuse, driver-bug cascades
and thermal events.** Fail-state nuance: **you can *win* the level (hit the revenue target) and
still lose the game** — bankrupt on depreciation when the bubble cools mid-level.
**The LOI variant:** customers queue with letters of intent at absurd prices, each wanting 500 GPUs
delivered in 90 days. Overbuild and ride the wave, or the bubble bursts mid-debt-term and you are
holding a million dollars of idle silicon on a five-year loan. Teaches **capex timing and demand
cycles** — and hands off directly into `Crypto Winter`.

### `Green Datacenter`
A constraint puzzle with no enemy. **Archetype: Tune. Length: Interlude.**

**How it works:** Meet a **PUE or renewable-energy target while staying profitable.** Cooling
buildables and power quality become the whole puzzle. A palate-cleanser level that quietly teaches
the facility layer's economics before a level that will charge you for not knowing them.

### `The Open-Source Fork`
A competitor forks your own in-fiction open-source tooling. **Archetype: Negotiate. Length:
Scenario.**

**How it works:** The fork creates a rival that **specifically knows the player's infrastructure
patterns** and counters them far more precisely than generic competitor sabotage. Your published
tools become your exposed playbook — the one level where *generosity* is the attack surface.

### `The Fire`
A hall goes up. **Archetype: Triage. Length: Siege.**

**How it works:** Mid-level catastrophic physical event, modelled on the Strasbourg datacenter fire:
**halon gas pours, staff evacuate the aisle in single file**, and you **lose territory instantly.**
The level hard-pivots into disaster-recovery gameplay — a race to re-serve from backups and other
sites — **while ransomware scammers and competitors pile onto the wounded.**
**The teaching moment:** *backups you never restore-tested half-fail.* This is the defining
backup/DR level in the catalogue, and the one that retroactively prices every restore-test tower the
player skipped.
**Interacts with:** §1.5 `The Neighbor Fire` (the collateral version where you *don't* burn),
§1.5 `Ransomware Sunday`, §1.5 `The Decom`, §1.11 The Rebuildability stat.
...
- **SLA modifiers as the player-tuned difficulty selector.** Before a run, sign customer SLAs: 99.0%
  vs 99.9% vs 99.99% uptime, payout multipliers, response-time clauses. Diegetic by construction —
  **difficulty as a business decision** rather than a menu setting.
- **Keep the dial, add drafting.** Each SLA tier comes with a **contract card** — measurement,
  exclusions, cap, credit window. The real dial isn't 99.9 vs 99.99, it is **what you are
  contractually defining as "down"**, so difficulty selection becomes *lawyering.* Pairs with the
  claim-based SLA credits and player-drafted measurement definitions in §7.

#### Punishment side of the SLA dial — *CONFLICTING*
Whether opting into a harder SLA should also raise the penalty. The decision turns on how a player
reads a difficulty they *chose*.

**Position A — reward and punishment both scale** *(master, and opencode wave-1 game designer)*:
higher SLA = more money per customer **and far less tolerance for error.** Elegant because it is
diegetic and it moves both the reward and the failure threshold at once — a self-balancing dial.

**Position B — scale the reward, cap the punishment** *(opencode, wave-2 game designer)*: opting
into a harsher SLA that *also* raises the penalty is a **double-loss trap**, and punishing players
for the difficulty they chose reads as cheating. Keep the reward side — higher rates, whale pools,
better contract offers — and soften the punishment to **capped payouts**; or invert it entirely, so
that **signing a lax SLA is the easy mode with a lower ceiling.** Players should feel they bought
*upside*, not a bigger stick.
...
- **Constraint cards and scenario-modifier decks (custom/replay mode).** Draft **3 modifiers** before
  a run: *"Only open-source stack (cheaper, weaker patches)"*, *"IPv6-only (fewer bots, fewer
  customers)"*, *"One-region (no failover)"*, *"All staff are contractors (no loyalty)."*
- **Two random modifiers per replay.** Every replayable level draws two: *"supply-chain discount but
  a counterfeit PSU batch"*, *"viral tweet: 3× customers, 3× script kiddies"*, *"heatwave: cooling
  capacity −30%"*, *"new compliance regime: all data in-bounds."*
- **"Cursed Configs" — stackable flags with score multipliers.** *Prices Locked* · *No Logging*
  (threat telegraphs disabled) · *Founder is a Maniac* (staff costs doubled, morale doubled) ·
  *Single Uplink* · *Everything's PHP 4*. The replayability engine and the community-event format
  (a weekly curse combo).
- **Modifiers stack with visible icons on the level card** ("Hurricane + Black Friday + SLA Death
  Spiral"), the daily challenge is a **seeded random combo**, and the community votes on the weekly
  one — endless content without new levels.
- **"One Uplink" as a weekly constraint-challenge format.** Run a normal hosting type under one
  absurd constraint — one uplink, no firewalls, must accept every customer, a budget cap — which is
  scoreboard bait that reuses existing content for nothing.
- **"Bootstrapped vs Backed" as a run modifier chosen at scenario start.** *Bootstrapped* = slower
  growth, no investors, a better ending if you get there. *Backed* = cash injections, forced growth
  metrics, board-interference events, acquisition endings. A replay hook across the same level set.
- **"Friday Deploy" hard variant.** Hazard-stripes the whole palette, adds a ticking CRT "Friday
  17:45" clock, and **doubles every bad-deploy probability** — the joke being that the UI itself
  panics, with alert icons starting to vibrate slightly.
...
- **Difficulty via entropy (the ambient-rot school).** Higher difficulty doesn't add enemies — it
  adds **ambient rot**: aging drives, flapping links, staff mistakes. Authentic, because real ops
  dies by a thousand cuts rather than by dragons.
- **Name the two dials and stop arguing.** Ship **PRESSURE** (wave composition and count) and
  **DECAY** (ambient rot rate) as two *independent* dials, with a recommended preset pairing per
  named difficulty tier: **"Shared Hosting"** = pressure-heavy, low decay · **"Enterprise"** =
  balanced · **"Production on a Friday"** = decay-heavy. The §10 difficulty-naming idea then slots
  the dials into the UI for free.
- **Add a third dial and the tension becomes a matrix.** **SURPRISE rate** — novel threat classes per
  level — is the missing axis; give each hosting type a **recommended triangle preset** shown at
  level select, and the flagged contradiction stops being a contradiction.
- **Every decay event needs a visible precursor.** SMART warnings, the LED tell the game already has,
  and **deterministic seeded windows**, so entropy is a readable clock a good player learns to
  schedule around. Random-feeling rot produces *"why did I die?"* rage, not mastery: **entropy is
  great on hard and terrible on normal** until the player knows the tells.
- **The four in-level scaling knobs.** Within every level: (a) attack-wave composition richness,
  (b) customer patience multiplier, (c) capital grant vs bootstrapped start, (d) **the number of
  simultaneous failure axes allowed.**
- **Difficulty change is behaviour change, not stat inflation.** Harder modes add **new behaviour
  rows**, not bigger numbers — *Standard:* hackers brute-force; *Hard:* they also spoof customers;
  *Nightmare:* they plant slow backdoors only the audit overlay reveals. Difficulty changes the
  puzzle, which is the same principle the level-diversity rules run on.
...
- **"The Pitch Meeting" (between-act interstitial).** A light meta minigame: pitch investors for a
  seed round where **your act performance *is* the pitch deck** (customers served, uptime, profit).
  Better numbers → bigger starting capital next act, which connects scoring directly to the
  progression economy.
- **Doctrine cards.** Spend earned **equity points** on company-wide doctrine (TLS-everywhere,
  mixed-vendor policy, no-Friday-deploys) that persists through an act and quietly retunes
  probabilities everywhere. **Doctrine as the macro layer above the tech tree**, with each card
  carrying a visible drawback clause.
- **The board meeting as a scene.** An alternative framing where the meeting hosts **major branching
  unlock decisions** — an in-fiction scene with named stakeholders arguing for different priorities,
  not only a doctrine-card spend screen.
- **"Season Mode: The Year in Hosting."** One level = one year, quarters rotating scenarios — Q1
  launch spike, Q2 breach, Q3 audit, Q4 holiday surge **plus the hardware-refresh deadline** (the
  budget must be spent before fiscal year end, whether or not you need anything).
- **"A Year in Support" (the rest-stop node).** Between big levels: 12 months of small tickets, two
  renewals, one hardware refresh, and a single weird event. **It measures your automation** — with
  runbooks it is a 60-second zen chore; without them it is a grinding week.
...
- **Sell and restart with a founder's cache.** Bank the score, start the next campaign with
  persistent perks **but harder market conditions** — standard 4X prestige with a sardonic
  acqui-hire epilogue.
- **One permanent unlock per acquisition.** Winning a run resets it but **carries one permanently
  unlocked buildable tier or tech-tree node** from each acquisition, so a prestige loop accumulates
  vocabulary rather than raw power.
- **"Relaunch."** Between campaign arcs, spin out a startup: bank reputation into a fresh run with
  better starting unlocks but reset infrastructure — **the tech tree persists partially, as
  institutional knowledge.**
- **"New Company, Old Grudges."** Restart with a reputation bonus and unlocked buildables, **BUT the
  attacker faction you most humiliated** (your top-2 recurring boss) remembers you, appears earlier,
  and brings new tricks. Meta-progression with escalating pushback rather than escalating comfort.
...
- **The "Zero-Day" interstitial (boss-wave framing).** Before a level's final assault the screen
  **glitches into a full-screen title card styled like an incident tweet or a status-page outage.**
  The green/yellow/red status dot becomes the campaign's chapter marker, and the level result stamps
  the page **"RESOLVED"** — or leaves it as a recurring incident.
...
- **"Endless Uptime Mode."** Escalating waves forever, but **with an economy**: you accrue debt-free
  profit and score = uptime-days × MRR. A leaderboard mode for build creativity, with the attacker
  AI holding the full codex unlocked from the first minute.
- **Per-level endless extension.** After winning any level, an endless wave-sim continues **from your
  exact build**; the leaderboard is cumulative uptime × revenue. The same map, reused as arcade
  mode, at zero content cost.
- **"Endless Uptime Week."** One persistent seven-day accelerated sim in which incidents roll
  procedurally like weather; score = final uptime × profit; and **ghosts of other players' weeks**
  are visible as a faint status overlay. No level boundaries — a pure skill-ceiling mode.
- **"Endless Ops" (sandbox).** Permadeath survival with escalating incident frequency; the
  leaderboard ranks months survived and EBITDA; and a **daily seed** gives everyone the same
  incident sequence — the "war story of the day" social hook.
- **"The Long Tail" (operator sim).** No scripted events at all: markets drift, competitors price,
  customers churn and arrive, hardware fails. **Score is ten-year company value.** Built for
  streamers and "my host vs yours" challenges.
- **"Black Friday Endless" (score attack).** The traffic curve spikes every five minutes and never
  stops, and **threats scale off your own traffic** — bigger payout pools attract bigger attackers.
  It is a graph of your greed: build more capacity and more bots arrive. Ends when the nines run
  out; a 60-real-minute variant scores on final MRR, and a net-worth-scaled variant makes the
  richer you get the more you are attacked.
...
- **Retro levels as history lessons.** The campaign loosely tracks real hosting history — dial-up →
  LAMP shared → colo boom → cloud → CDN/streaming → AI gold rush — and **each era's *joke and
  trauma* is the scenario**: IPv4 exhaustion hits exactly when you go public; a flash crowd kills
  your first game client. Players finish the campaign knowing *why the industry looks like it does.*
- **Gate the threat catalog by era, mechanically.** Each era's waves draw from a period-authentic
  threat set (phreaks → worms → DDoS → APT → prompt-injection), and **discovery-tree edges are
  era-locked too** — researching a counter *before* its threat arrives is the New Game+ era-mode
  reward. This makes the retro spine and the era New Game+ mode share **one** system.
- **Ship it as an era-preset pack.** One selector swaps HUD chrome, palette, sound, threat catalog
  and customer archetypes **simultaneously**, so every other level (and the sandbox) can be replayed
  in any unlocked era — history as a **playable skin system**, not a set of fixed levels. This also
  resolves the scattered era-HUD and bezel entries into a single system.
- **"Y2K: The Reckoning" (New Game+ era mode).** The same maps across eras — mainframe timeshare →
  dial-up → web 1.0 → cloud → AI — each with its own threat catalog, and **the era tech tree
  persists in meta-progression.**

### The Nine Tiers (uptime "lives" as the progression spine)
Lives, denominated in nines.

**How it works:** Each level grants uptime "lives" expressed as **nines**: you start at 99.9% — one
nine to spare — and a breach, an outage or a churned whale **eats a nine.** Hitting 0% fails the
level, and **each restored nine buys back a small percentage of SLA-credit liability**, so the lives
bar and the money meter are the same object seen twice.
**Interacts with:** §1.7 The Nines Ceiling, §1.7 The Failure Ladder, §7 (SLA credits), §2 (threat
cost model).

### Reverse-gate progression
Some branches unlock only by having been hurt.

**How it works:** Certain tech-tree branches are locked behind **having suffered** the matching
attack type — you can only research ransomware recovery *after* a ransomware incident. It quietly
encourages players to occasionally **accept risk in order to unlock**, which is the opposite of how
every other gate in the game behaves, and is why it is worth having exactly one of.
**Interacts with:** §6 (discovery), §1.6 Achievement gates.

### Reputation Echo chaining
Each level's grade rewrites the next level's opening conditions.

**How it works:** **3★ → customers start closer to your gate and a referral bonus wave fires.
1★ → spawns are farther out and churn vultures are already circling.** Campaign momentum becomes a
legible rising or falling **"market buzz" meter** on the roadmap screen, so a good run *feels*
cumulative without any stat being inflated.
**Interacts with:** §1.11 (the roadmap/level select), §1.6 Scars, §4 (attraction).

### Three-star meter = Uptime / Profit / Retention
Every level grades three independent axes, not one.

**How it works:** A level can be 3-starred for profit and **0-starred for retention.** Star gates on
the tech tree therefore force replay with different strategies, which pulls players into the dual
flow deliberately rather than hoping they notice it.
**Interacts with:** §1.6 Cohort View, §7 (scoring), §1.11 (level select grades).

### "New Game+ Compliance Mode"
Replay the early levels with your mature toolkit — and pay for it.

**How it works:** Every toy you deploy **retroactively attracts tailored attacks**: your tech tree
itself widens your threat surface, so the mature player walking back into the closet level finds it
is no longer the closet level. The cleanest statement in the document of the game's core law that
**capability is surface.**
...
- **The Pulse Board (an invisible director for the budget).** A hidden tension governor samples
  **"panic proxies"** — click rate, actions-per-minute of fixes, bounced-customer count — and nudges
  the *next* wave's composition toward a target tension curve: a bored player gets a spicier combo,
  a drowning player gets one softened wave. **The diegetic skin announces itself as "market
  conditions," never as RNG**, and it is strongest in endurance levels where an authored curve
  cannot know how tired the player is.
...
- **Difficulty as a news cycle (the third, global axis).** Express the global difficulty knob as a
  **live tech-news ticker**: each headline ("IoT botnet source code leaked") **primes a threat family
  for the next N minutes.** Harder modes are simply an angrier news cycle — so the player can read
  the difficulty setting off the world instead of a menu, and the ticker doubles as a telegraph
  layer.
...
- **"Failure teaches" — the postmortem that unlocks.** Every game-over produces a **blameless
  postmortem screen naming the exact counter you lacked**, and *unlocks it for purchase next
  attempt.* Dying becomes guaranteed forward progress, and the design rule behind it is absolute:
  **never lock players in a wall they have no vocabulary for.**
- **"The Post-Mortem Dungeon" — your failure museum becomes playable.** Every run you LOSE converts
  into a **fixed-path corridor level**: your failure's exact timeline, fast-forwarding and halting at
  each mistake node. Execute the correct action from your codex to pass; clearing it awards the
  **runbook card** for that incident *and* a permanent discount on the counter tech.
- **"Hall of Shame" ghosts.** The game records your *actual* worst incidents — the config that took
  you down, the restore that failed — and **replays them as attack waves in later levels and in your
  friends' games.** Your 3am is everyone's content.
- **Ghost-of-Runs-Past on replays.** Replaying a level for stars shows a **translucent ghost of your
  previous run's state at the same clock point** — where the last you was on fire, what the last you
  had not yet built. Learning made spatial and comparative; pairs directly with the failure museum.
...
- **Chapter palette shifts.** Every campaign act recolors **the UI chrome itself**, not just the map:
  dial-up = beveled window-gray with bitmap fonts; cloud = flat dark with subtle gradients; the AI
  era = charcoal with orange edge-glow. **Players feel progression in the furniture.**
- **Scenario colour-grading presets.** Every scenario archetype ships its own screen-wide look-dev, so
  a launch spike *feels* different **before you read the objective**: spike = warm high-key bloom
  plus a ticking vignette · breach = desaturated, red fault lines, film grain · audit = flat, cold,
  shadowless "inspection lighting" · hurricane = rain-streak, low contrast, lightning legibly mapped
  to power state · migration = split-screen duotone, old amber against new cyan.
...
### "Panic Button" levels (designed to lose)
Bonus scenarios that are **mathematically unwinnable** — and say so.

**How it works:** The 10/10 attack with zero scrubbing budget; the Cat-5 hurricane with no generator.
You are scored on **grace under pressure**: survival minutes, communications quality, post-mortem
accuracy, customers evacuated. It feeds the failure-teaches loop **without the frustration of "fair
but brutal"**, because the contract with the player is honest from the first frame. Best used as act
interludes.
**Interacts with:** §1.7 The Failure Ladder, §1.7 Soft failure over hard failure, §1.5
`Hurricane / Regional Event`.

### Scale-Skip challenge levels
Start cold at tier 4 with a pre-built, bad datacenter.

**How it works:** Inherited upkeep debt, no bootstrapping, **pure ops-triage rhythm from the first
second** — a completely different opening tempo from the campaign's grow-from-nothing beat. It also
doubles as the generator for every "legacy debt" variant: any tier-4 board plus a bad inheritance is
a level.
**Interacts with:** §1.11 The Cold Open Level, §1.5 `Legacy Debt`, §1.6 Ratchet unlocks.
...
- **Level = Scale × Type × Constraint (the procedural scenario grammar).** A level is built from a
  triple: **hosting type** (12+ options), **scale tier**, and **one scenario constraint card** ("no
  firewall", "budget cap", "launch day", "solar flare forecast"). The campaign is generated from the
  grammar, and challenge modes roll exotic triples.
- **What each axis owns.** **Scale tier** changes the board — grid size, power budget, uplink
  capacity, ingress points. **Hosting type** changes the threat table, the customer table, the
  buildables and the visual skin. **The scenario** adds one goal or constraint that bends the rules.
  Nothing else is allowed to vary, which is precisely what stops fragmentation.
- **Goal archetypes to mix per mission**, so that "win" never feels identical twice: survive N waves
  · serve X customers total · end with $Y net worth · keep churn under Z% · hold uptime ≥ 99.9% ·
  pass inspection at waves 5/10/15 · contain the breach by wave N.
- **The level matrix drawn as a level-select rack.** Each slot is a (type, scale, goal) mission:
  unplayed slots are dark, cleared ones glow and show your grade as an **LED colour**, and replays
  re-run with modifiers. The grammar becomes furniture the player can read at a glance.
...
- **Escalation-curve rules (the novelty budget).** Each level introduces **exactly ONE new threat
  family, ONE new customer archetype, ONE new buildable line** — everything else is remix, and
  designers audit novelty per level against the roster.
- **Threat scaling keys off MRR, not defenses.** Wave composition scales off **your revenue**, so
  hiding (staying small and cheap) is viable but low-scoring: the game rewards *managed* greed
  rather than punishing ambition directly.
- **The visual-complexity budget escalates too.** Later levels raise the number of entity species,
  crossing lanes and simultaneous overlays — with **at most two new visual languages introduced per
  tier**, so the screen gets busier at the same rate the player gets fluent.
...
- **"Car-Hit-The-Pole" micro-levels (the bucket below Interlude).** 2–3 minute vignettes named after
  real causes of outages — **backhoe fade, bird in the intake, the admin's cat on the keyboard,
  "Friday 17:58 deploy"** — each built around one mechanic, sharply executed. They are ideal for
  loading screens and for interspersing with the tutorial, and they give the length taxonomy a floor.
...
- **The Five-Beat Level (the dramatic-arc law).** Every level template guarantees the same arc:
  **Probe** (one readable threat, no customer risk) → **Learn** (a second family appears, forcing a
  counter choice) → **Escalate** (two families combo) → **Commitment Test** (a boss stacking all
  present families) → **Denouement** (a guaranteed 30–45s calm where the economy pays out and
  tidy-up is rewarded). The wave composer **validates arcs against the grammar**, and scenario
  variety lives *inside* the beats rather than instead of them.
- **"One Day in the Life" as the universal four-act bar.** Morning traffic ramp / noon deploy /
  afternoon incident / evening quiet, compressed into ~20 minutes. Codify it as **the pacing
  template every scenario level maps onto** — even Hurricane and Audit: it is really the *grammar*,
  not one level among many.
- **The day/night heartbeat underneath it.** Between daytime traffic waves, **night brings scanners
  and maintenance windows** — night is build and repair time (cheaper builds, no customers) — and
  the cycle renders diegetically: ambient light cycles, the warm customer flow thickening at
  "evening", room lights dimming on eco-mode. Launch spikes then read as a sudden tide against a
  known rhythm.
- **Diurnal load as an economic level ("Peak Hours").** 7pm flood, 4am crickets: prepaid capacity vs
  spot rental of a neighbour's idle servers. **Gamers churn instantly when tickrate drops at prime
  time and nobody cares at 5am** — yield management, taught by the clock.

### The Gated Syllabus rule
The curriculum is explicit, and bosses never teach.

**How it works:** **Every act ends with a competence-check level requiring exactly the verbs that act
introduced**, and one boss that stacks them. **No new system ever lands in a boss level.** The
result is that pacing never spikes on an unfamiliar verb — every difficulty spike is a spike in
*execution*, never in vocabulary.
**Interacts with:** §1.9 The Level Archetype Taxonomy, §1.6 Chapter bosses, §1.9 The Invariant Core.

### QA grammar: the one-frame thumbnail test
A shipping gate stated as a picture.

**How it works:** **Every level's thumbnail at 128px must communicate hosting type, scale and
scenario with no text.** It governs every level's visual budget from the first concept sketch, and it
doubles as store-page art direction — if the thumbnail cannot carry it, the level's identity is not
yet designed.
**Interacts with:** §1.8 (tier postcards), §1.10 The Scenario Icon Family, §1.11 (level select).
...
- **"Night Watch" (the wave-less shift).** Threats and customers dribble continuously and a
  paging system wakes you across a sleep cycle: each alert answered well is a bonus, each missed one
  a consequence. **Attention IS the resource**, which is the purest expression of what a shift is.
- **Night as build time.** Between daytime traffic waves, night brings scanners and maintenance
  windows — **builds are cheaper and there are no customers to disturb** — so the day/night cycle is
  an economic rhythm, not only a lighting one.
...
- **The contract card is the real dial.** Each SLA tier arrives with a card specifying
  **measurement, exclusions, cap and credit window** — because the meaningful choice is not 99.9 vs
  99.99, it is **what you are contractually defining as "down."** Difficulty selection becomes
  lawyering, and the score screen inherits the vocabulary for free. (See §1.6 `Difficulty as SLA`
  for the dispute over whether the penalty side should scale with the reward side.)
...
- **Scenario-stamp intro cards — one card, one sentence, zero tutorial popups.** Every level opens as
  **a diegetic artifact matched to the scenario's world**: hurricane = a barometric chart pinned to a
  wall · audit = a certified-mail envelope · breach = a pager printout still warm · migration = a
  landlord's eviction letter on stationery · the invoice level = **an actual bill sliding under the
  door.** Era fonts carry the tone; it pairs with the bezel-UI law.
- **Poster-card variant.** A full-screen pulp-tech illustration of the scenario with a bold
  display-type title and constraint badges drawn as stickers — **"NO CLOUD", "SLA 99.9%"** — which
  doubles as the level-select gallery: the player's wall of gigs collected.
- **Manila-folder variant.** The brief is an on-screen object in-fiction — a support ticket, a glossy
  pitch deck, a subpoena, **a bar-napkin sketch for the garage-startup level** — and the document's
  art style telegraphs difficulty and tone before a word is read.
...
- **Mid-level Commitment Forks.** At **40% of long levels** a scripted decision offers two
  *irreversible* paths — "scale up in place vs open region B"; "accept the shady whale vs keep the
  clean tail." **Each fork visibly re-skins lanes, threat lists and the money curve for the rest of
  the level**, which gives the genre a big-choice texture it almost never has, and gives replays a
  reason.
...
- **The two canonical two-board shapes.** **`Two Maps, One Level`** (§1.4) splits the *same* facility
  into a SERVER ROOM view and a NETWORK view joined by a toggle, so threats can live on one layer or
  both. **`Two-Board Duo Levels`** (§1.4) splits the *company* into a public-facing edge board and an
  internal back-office board (billing, staff, backups), where a missed payroll silently weakens the
  edge. The Double-Header's shared-budget rule applies to both.
...
### Architect Mode Pause
Pausing is not a freeze — it is a change of drawing.

**How it works:** Pausing flips the whole world into **blueprint/CAD rendering**: white-on-blue line
art, dimension callouts, entity specs as drafting annotations. Building **while paused in CAD** is
allowed on relaxed difficulties, which turns the pause menu into the game's planning tool rather
than its escape hatch.
**Interacts with:** §1.8 The Blueprint Rewind, §1.7 (difficulty-tier permissions).

### Phase-adaptive HUD (shift-change UI)
The HUD reflows like a NOC change-of-shift.

**How it works:** Between build and combat phases the interface rearranges itself: **the catalog
palette slides in during build, the alert stack and pager expand during ops, and both shrink to
ambient during breathers.** It answers "where do I look" *per phase*, instead of shipping a
permanent wall of panels that is wrong two-thirds of the time.
**Interacts with:** §1.7 The density/interface lockstep rule, §1.10 The Constraint Band.

### Starter Loadout = class select
Three prebuilt micro-stacks, chosen at level start.

**How it works:** Pick one of **Lean Static, Dynamic Duo, or Fortress Paranoia** — each biasing early
buildables, upkeep, and **threat-attract profile.** It is the tower-defense answer to "same map, six
ways to play," and it doubles as a teaching device that contrasts three architectures side by side
without a single tutorial line.
**Interacts with:** §1.10 The Signed Contract, §1.6 Difficulty as Business Model.

### Contract Market draft (between levels)
Choice architecture, moved to a calm screen.

**How it works:** Before each level, **draft 3 of 5 offered customer cohorts** from a refreshable
market board. Your picks set the next level's catalogs for **both** flows — gamers bring booters,
shops bring fraud bots — so the most consequential decision of the level is made somewhere it can be
*deliberated*, rather than shouted over by waves.
**Interacts with:** §1.10 The Signed Contract, §4 (customer archetypes), §1.6 Customer Quality Mix.
...
- **The Roadmap Campaign (choose-your-path).** Replace the linear level list with a **branching
  calendar/path map** (Slay-the-Spire shaped): at each month-node, pick from 2–3 scenarios with
  **visible reward tags** — "this node teaches power," "this node is money-heavy." Difficulty then
  emerges from *ordering* and replayability from *route variety*, and the campaign screen is a
  diegetic **wall planner in the HQ** rather than a menu.
- **The Franchise Map (the between-campaign layer).** Progression becomes picking **WHERE your next
  company lives**: a strategy-layer map of countries and states as **jurisdiction cards**, each
  carrying legal modifiers (data retention, raid risk), a power price, a customer-pool flavour and a
  threat bias. Certifications gate which cards you may play, and **the same scenario under a
  different card is a different level.**
...
- **"The Cold Open" as a reusable level type (in medias res).** A level that starts **mid-meltdown**
  with a pre-built, broken datacenter: **60 seconds to stabilize, and only then does the tutorial
  quietly explain what you just did.** Teaching by retroactive narration.
- **"Everything Is On Fire" variant.** The previous admin quit, you are breached, and you restore
  from backup **while under active attack** — incident response that reverses the usual build-up
  pacing entirely.
...
- **"Daily Incident" (the roguelite run).** A **fixed seed, a shared scenario, and one unusual
  constraint** — "you start with one IP address", "patching costs double" — with the leaderboard
  resetting daily. A cheap content engine that keeps the meta honest, because everyone is solving
  the same board.
- **The weekly variant ranks on a ratio, not a total:** **"nines held per dollar"** on the same map,
  which is what produces global meta-solutions worth arguing about rather than a spending race.

### Level-select presentation — *CONFLICTING*
What the campaign screen *is*, as an object. All three candidates are diegetic and all three are
good; they are mutually exclusive because the campaign screen is the single most-seen surface in the
game and it can only be one thing. The decision turns on whether the artifact should record **your
route** or **your company's public face.**

#### Position A — the patch panel (and the job board beside it) *(master)*
The campaign map is a **patch panel on the office wall**: each level is a labelled port, completed
levels have a cable plugged in, branches are visible as where cables can still go, and hosting-line
unlocks add a whole new panel row with its own colour band. The Line Draft deals three unplugged
cables and you choose which port to seat; **the cable you plug in stays plugged for the rest of the
run, so the panel is a record of your route.** Scenario levels hang beside it on **the Job Board** —
a corkboard of printed tickets, work orders and faxes with era-appropriate letterhead, completed ones
spiked on a bill spike in the corner. An expanded reading makes the run's progress a **hand-dressed
cable run, jack to jack**: locked levels are empty holes, cleared levels lit ports, perfect
three-star runs **gold-plated jacks**, skipped optional levels visibly un-cabled — RJ11s in the retro
acts, LC fiber in the cloud acts. Your save file becomes a cable-dressing artifact people screenshot.
*(§1.11 also offers a third master variant — the Retrospective Level Select, a timeline of your
company that rewrites itself when you replay a level for a different outcome.)*

#### Position B — your own website, upgrading as you win *(opencode)*
The level select is **a mock of the company's public site that upgrades as you win**: a GeoCities
hand-edited page at Level 1, Web 2.0 gloss mid-game, a clean docs-site with a live status page at
endgame. **Completed levels are literally pages on it**, your SLA feeds its green badge, and each
hosting type skins it — a game host gets a dark forum, a regulated host a sterile government portal.
The argument for it is that it makes progression *externally visible*: the patch panel records what
you did, the website records **who the world thinks you are**, which is the stat the rest of the
game keeps scoring you on.

### End-of-level presentation — *CONFLICTING*
How a level's result is delivered. The dispute is whether the result is an **artifact you keep** or
**a tour you watch**, and it matters because the chosen form is also how the game accumulates
memory.

#### Position A — the report card as an in-world artifact *(opencode, wave-3 visual)*
Each level ends by **"printing" an artifact**: a status-page screenshot pinned to the wall, a Grafana
dashboard PDF sliding across the desk, a framed ISO certificate, an arcade HIGH SCORE board for game
servers, a faxed incident ticket for email hosting. Stats render **onto** the artifact, and the office
wall accumulates your framed history — which dovetails with §1.8 The Company Wall.

#### Position B — the real-estate listing tour *(opencode, wave-2 visual)*
The level-end camera does a slow **listing-video sweep of your facility**, stat pins popping as it
passes each system ("Web tier: 99.98% this month"), ending on a parody review-site grade screen. It
is the whole-base property tour — deliberately distinct from failure-highlight documentaries — and it
shows the player *the thing they built* rather than a page about it.

### Endless and post-campaign modes
Where the campaign stops and the operator sim starts.

**How it works:** See §1.6 `Sandbox unlock parity` for the full roster — Endless Uptime Mode, the
per-level endless extension, Endless Uptime Week, Endless Ops, the ten-year operator sim, and Black
Friday Endless. The structural point for campaign topology is that **every one of them reuses an
existing map**, so post-campaign content costs art time only where it costs nothing.

### "Finale: The Split" (the branching campaign ending)
The final act forks into two mirrored campaigns.

**How it works:** Become **the ethical giant** — regulated tiers, audit bosses, compliance as the
endgame content — or **the bulletproof kingpin**, where abuse traffic *is* revenue and law
enforcement is the boss family. **Same mechanics, mirrored threat and customer tables.** The two
endings replay the whole game from opposite sides of every moral choice the campaign has been
quietly tallying.
**Interacts with:** §1.5 `The Offshore Gamble`, §1.6 Difficulty as Business Model, §1.11 The Pivot
Map, §1.1 "Founding Day" (which is where the fork was actually chosen).
...
- **"The Postmortem-Driven Level."** The level **opens with a summary of a past (fictional) incident
  at a similar company** and then specifically tests whether the player learned that lesson —
  *before* it happens to them too. Prevention-through-reading-the-report, used as the scenario frame
...
- **"Demand Response" as its own level, with the economy inverted.** The grid declares peak urgency
  and **bills you — or pays you**: a curtailment meter forces you to shed load (gracefully drain
  noncritical tenants, brown out your own lanes) or buy premium power. The late twist is that your
  batteries and powered-down racks **sell capacity back to the grid for real money**, so for one
  level the defense economy runs backwards and *doing less* is the profitable play.
...
- **The elasticity framing.** Raise prices to fix margin and **watch elasticity per segment**: churn
  spike against revenue lift. Grandfathering old customers costs you; nuking them costs reputation.
  *The whole level is a spreadsheet that fights back.*
- **"Renewal Shock."** You grew on **$0.99 first-year bait** and a whole cohort hits its first
  auto-renewal at 10× price **in a single week**: a churn wave, a review swarm, and a
  consumer-protection regulator spawning if the disclosure was buried. Grandfather them, phase the
  reprice, or goodwill-credit the cohort. **Teaches that teaser pricing is a loan from your future
  churn curve.**
- **"The Grandfather Problem."** You inherited 500 accounts on 1998 pricing — $3.95/mo with 2 GB and
  a personal web page — while cost-to-serve tripled and churn-on-price-hike sits at 40%. Reprice,
  migrate, or **buy them out** over 18 game-months without triggering a mass cascade, while churn
  ghosts poach live customers. Achievement: *"Grandpa's Rate."*
- **"Click-to-Cancel."** An FTC-style crackdown on dark-pattern retention: a hidden **enforcement
  meter** maxes out and a probe arrives — strip every maze screen, honor cancellations in two
  clicks, refund within the window, all while normal ops continue. Surviving with margin intact
  grants a permanent **"Honest Co." attraction buff.** *The villain is your own P&L.*
...
- **See §1.5 `The Offshore Gamble` / `The Chain of Custody` / `The Subpoena Raid`** for the full
  scenario treatment of this business line — the seizure meter, the raid as an unstoppable unit you
  only prepare for, jurisdiction-as-placement, the lights-out cage seizure, and the hidden
  "Grey Market" unlock where getting *too good at uptime* churns your customers upward to
  respectable providers.

### The Churn Cliff
The operator's real disease, made into a level with no boss at all.

**How it works:** No attack. **MRR erodes 1–2% a month** (a harsher variant runs 4%) unless the
player invests in retention — support quality, loyalty credits, shipped features — and in late
levels the cliff **steepens** as enterprise procurement refresh cycles land together. The gameplay is
diagnosis: figure out **WHY** (tickets piling up, latency drift, a price increase, one bad review
thread) and reverse the curve in time to survive to month 18. **Anti-climactic on purpose.**
**Interacts with:** §1.6 Cohort View, §1.12 Price Increase Day, §4 (customer patience).

### The Churn Auction
A rescue scenario in which the waves are departures.

**How it works:** The level starts at **5%/mo churn** with a board demanding the curve bend within
six months. **Each month grants exactly one lever** — fix response times, ship a demanded feature,
reprice, launch winback, fire the chatbot — and cohorts **visibly change behaviour** in response. A
diagnostic reverse-tower-defense: you are not killing attackers, you are preventing exits, and the
scarcity of levers is the whole puzzle.

### Peak Season Yield (airline mode)
Revenue per rack, priced like seats on a plane.

**How it works:** Maximize **revenue-per-rack across Nov–Jan** by mixing month-to-month survivors,
reserved commitments, and spot-priced overflow. Yield management as an endgame spreadsheet skill
layered on top of the tower defense — the same capacity sold three different ways, and the skill is
knowing how much to hold back.
**Interacts with:** §1.6 Seasonality, §1.5 `Black Friday` / `Peak Season`, §7 (pricing).

### Tax Season (the SMB / accountant vertical)
A professional community that never forgives.

**How it works:** You host portals for accounting firms, and **Feb–Apr traffic triples every night.**
Under-provisioning means **your customers miss statutory deadlines**, and that customer class does
not forgive it. Teaches vertical seasonality — and, more usefully, **why niche markets pay a
premium**: they are paying for the night you do not drop.

### Academic Year (game servers and the education vertical)
September floods in, May walks out.

**How it works:** Freshmen arrive in a wave; dorms and esports leagues create brutal seasonality plus
**a customer class with zero budget and infinite entitlement tickets** — cheap CAC, cheap churn. A
comedy level about cohort seasonality carrying a real lesson in **staffing elasticity**: you cannot
hire for September and fire in May without it showing up in your support quality.

### The Registrar Grind
Run a domain registrar for a level. Razor-thin, hardware-free.

**How it works:** **Wholesale $10.26, retail $9.99** — you only win on renewals, privacy upsells,
transfers-in, and speculators buying 40,000 names at once. ICANN fees, 60-day transfer-lock exploits,
**zero hardware.** A retro/economics tutorial that teaches unit economics by removing every
infrastructure lever the player has been relying on.
**Interacts with:** §1.2 `The Registrar` / `NXDOMAIN`, §1.5 `Retransfer`, §1.5 `The Lame Delegation`.

### The Product Sunset Dilemma
Legacy-margin addiction, stated as a choice.

**How it works:** Your oldest SKU has **2,000 customers, 70% gross margin, and one engineer about to
quit and take the knowledge with him.** Maintain, freeze-and-hike, or migrate-off — each path
spawning its own mini-wave of support tickets and churn events. The trap is that the *most*
profitable product is the one you least understand.
**Interacts with:** §1.12 The Sunset Letter, §1.5 `End of Support`, §1.5 `Legacy Debt`.

### Dot-Com Bubble
An economy level about **declining** money.

**How it works:** Irrational exuberance: capital arrives absurdly cheap (VC offers 5× normal),
customers arrive pre-revenue, and valuations outrun uptime. **The money spigot closes on a hidden
timer.** Every dollar spent during the bubble converts into **upkeep obligations that persist after
the burst.** Win = survive with non-negative cash; **score = how much free money you *declined*.**
Post-burst, survivors scavenge the corpses.
**Interacts with:** §1.5 `Crypto Winter`, §1.5 `The GPU Gold Rush`, §1.5 `Sell the Company`.

### Sell the Shovel (the post-breach pivot)
Your security stack becomes the product.

**How it works:** You got breached, built a great detection stack, and customers now ask to **buy
your SOC as a product.** Your security buildables become revenue generators serving NPC tenants —
**but YOUR stack is now their attack surface, and one slip is a class action.** Managed security as
a two-sided level, where every defensive investment you made is re-scored as a liability.

### Category Creator
Positioning as a timed moat.

**How it works:** Instead of climbing the shared rankings, **name a new market** — "the managed
Metal-as-a-Service host." Low initial spawn volume, **zero competition**, and content-marketing
towers at 3× effectiveness — until competitor AIs clone your category page two quarters later. The
level is about how long you can hold a definition.

### Founder Mode vs Manager Mode
A mid-campaign fork about what kind of company you are.

**How it works:** **Stay hands-on** — cheap, fast decisions, capacity-capped, because you personally
ARE the NOC and the sales team — or **institutionalize**: an org chart that costs money every tick
but unlocks enterprise deals that require "process." **Once institutionalized, some scrappy
early-game options close permanently. You cannot un-hire a CFO.**
**Interacts with:** §1.2 `Founder Mode` / "The CEO Chair", §1.6 The Ratchet, §1.11 (branching).

### The Regulator Sandbag
Greed produces a second resource drain.

**How it works:** **Every dollar over a threshold attracts a tax/audit tick** that must be "paid"
with paperwork built by your logging towers — so profit literally manufactures its own overhead.
Players learn to route around it via the cooperative/legal tier, and it pairs with the
offshore-vs-regulated contrast levels as the *legal* half of the same argument.
...
- **The Decade Dial.** Each era re-skins the whole presentation in one shot: **beige CRT plus
  dot-matrix sounds** (dial-up) · **web-safe colours and table-layout UI** (the shared era) ·
  **skeuomorphic gloss** (mid-VPS) · **flat minimal and dark mode** (cloud/GPU). Progression is felt
  as *taste evolution* — and **a high-contrast accessibility override must survive all eras**, which
  is the one rule the shader stack is not allowed to break.
- **"The Internet Grows Up" (the whole game staged as a timeline).** Levels 1–5 are 1995 (dithered
  chrome, dial-up sound design), mid-game is Web 2.0 gloss ("AJAX blue"), late game is hyperscale
  neon (glassmorphism dashboards) — and **unlocks arrive as period-authentic tech**: 5.25" floppies →
  tape → SSD → GPU.
...
*Modes in which a second operator — human or AI — sits on the other side of the board. They reuse the
campaign's maps and rulesets wholesale; what changes is who is holding the other end of the cable.
The single-player inversions of the same idea live in §1.2 as perspective levels (`The Attacker` /
"You Are The DDoS" and `The DR Site`), because there the camera moves but the opponent does not.*

### Two-Player Co-op: SecOps vs NetOps
One board, two operators, half a health bar each.

**How it works:** One player owns **defenses and detection** (and sees the threat layer); the other
owns **service and capacity** (and sees the traffic layer). **Each sees only half the health bar
clearly**, with a shared chat/ping tool — so **forced communication *is* the content.** Plays at
scale 3 and above, where there is genuinely enough board for two pairs of hands.
**Interacts with:** §1.4 `Two Maps, One Level` (the same split, given to two people instead of one
toggle), §1.9 The Invariant Core.

### Co-op split-campus mode
Two datacenters, one company, one contested wire.

**How it works:** Two players run two datacenters of the **same** company, and **the interconnect
between them is shared, finite, and must be negotiated in real time** — whose DDoS scrubbing detour
routes through whom? The level's drama is entirely about a resource neither player owns outright.
**Interacts with:** §1.1 Tier 5, §1.5 `Two Masters` / `Split Brain`.

### Rival Escalation Duel
A persistent competitor company on a parallel board.

**How it works:** You both serve the **same customer pool**. Each level you trigger **one aggressive
move** — a price war, a fake review campaign, a hiring poach, or a deniable DDoS — and defend against
theirs. The mode ends when one company's customer base hits zero. It turns the campaign's scattered
competitor events into a single running relationship with a memory.
**Interacts with:** §1.5 `The Copycat` / `The Price War`, §1.5 `The Ransom Customer` (the deniable
DDoS is the same object), §1.4 `Two Brands, One Datacenter`.

### Peer-vs-Peer "hosting war" level
PvP shape, single-player cost.

**How it works:** A competitor's board sits across a **shared transit cloud**: you poach their
customers with marketing towers aimed at the midline, they poach yours, and **DDoS reflectors fire
across both.** A PvP-shaped level driven by an AI operator — so it ships without matchmaking, and
the AI can be given the full unlocked codex from the first wave.
